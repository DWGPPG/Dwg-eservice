...
function bindReviewInteractions(view, state) {
  ...
  view.addEventListener("click", async (event) => {
    ...
    if (action === "confirm-box") {
      ...
      if (mode === "revise" && !source) { showToast("กรุณาเลือกสาเหตุการแก้ไข (1.1–1.4)", "warning"); return; }
      if (!note) { showToast("กรุณาระบุรายละเอียด", "warning"); return; }

      button.disabled = true;
      button.classList.add("is-loading");
      try {
        await requesterReview(request, mode, note, source, { files: getReviewFiles(requestNo), link });
...
function toggleReviewBox(view, requestNo, mode) {
  ...
  if (mode === "revise") {
    ...
    // ซ่อน dropdown สาเหตุสำหรับ Reject
