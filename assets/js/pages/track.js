import { STATUS, STATUS_LABELS, CLOSED_STATUSES , CANCEL_SOURCES, getCancelSource } from "../../../config/schema.js";
import { appConfig } from "../../../config/config.js";
import { openModal, confirmSameDayDueDate } from "../components/modal.js";
...
import { assigneeName, getDrawingTeamMembers, getKnownRequesters } from "../services/team-service.js";
...
// ══════════════════════════════════════════════════════════════
// ช่องค้นหา: ปัญหาเดิมคือ re-render ทั้งหน้าทุกครั้งที่พิมพ์ 1 ตัวอักษร
// ทำให้ input ถูกสร้างใหม่ → หลุด focus → พิมพ์ต่อเป็นคำไม่ได้
// แก้โดย: หน่วงเวลา 250ms ให้พิมพ์จบคำก่อนค่อย re-render
//         แล้วคืน focus + ตำแหน่งเคอร์เซอร์เดิมกลับไปให้ครบ
// ══════════════════════════════════════════════════════════════
let _searchDebounceTimer = null;

function bindSearchInput(view, inputId, onChange, rerender) {
  ...
  input.addEventListener("input", (event) => {
    ...
    // จำตำแหน่งเคอร์เซอร์ไว้ก่อน (รองรับกรณีผู้ใช้แก้กลางข้อความ ไม่ใช่แค่ต่อท้าย)
    ...
    _searchDebounceTimer = setTimeout(() => {
      rerender();
      const fresh = view.querySelector(inputId);
      if (!fresh) return;
      fresh.focus();
      try {
        fresh.setSelectionRange(caretStart, caretEnd);
      } catch {
        // input บางชนิดไม่รองรับ setSelectionRange — ไม่เป็นไร แค่ focus ก็พอ
      }
    }, 250);
  });
}
...
let trackDateFrom = "";
let trackDateTo = "";
let drawingFilter = "";   // กรองตาม Drawing No.
let columnStatusFilter = ""; // กรองตามสถานะ (column filter ในตาราง)
...
export function renderTrack(view, state) {
  ...
  if (role === "manager" || role === "designer") {
    ...
    // ลบ renderMyRequestsSection ออกแล้ว — รวม "รอตรวจรับ" ไว้ใน Tab งานของฉันแทน
...
/**
 * สิทธิ์ "โอนผู้ร้องขอ" — ใช้กรณีผู้ส่งคำร้องเดิมลาออก/ย้ายงาน
 * เงื่อนไข: เป็นงานที่ตัวเองรับผิดชอบ (หรือเป็นผู้จัดการ) และงานยังไม่ถูกยกเลิก/ตีกลับ
 */
function canTransferRequester(item, state) {
  const role = state.user?.role;
  if (role !== "designer" && role !== "manager") return false;
  if ([STATUS.CANCELLED, STATUS.REJECTED].includes(item.status)) return false;
  return role === "manager" || isMine(item, state);
}

function filterByStatusTab(items, state) {
  ...
  if (statusTab === "awaiting") {
    // รอตรวจรับ — งานที่ฉันส่งเองและสถานะ DELIVERED (รอ Requester กดตรวจรับ)
    const myEmail = getMyEmail(state);
    return items.filter((item) =>
      item.status === STATUS.DELIVERED &&
      String(item.requesterEmail || "").toLowerCase() === myEmail
    );
  }
...
function filterByDrawing(items) {
  if (!drawingFilter) return items;
  return items.filter((item) => item.drawingNo === drawingFilter);
}

function filterByColumnStatus(items) {
  if (!columnStatusFilter) return items;
  return items.filter((item) => item.status === columnStatusFilter);
}
...
function renderWorkbookTrack(view, state) {
  ...
  // ── Holiday set สำหรับคำนวณวันทำงานทั่วทั้งหน้า ──
  const holidaySet = new Set(
    (state.masterData?.holidays || [])
      .map((h) => h.holidayDate || h.HolidayDate || "")
      .filter(Boolean)
  ...
  const drawingOptions = [...new Set(state.requests.map((i) => i.drawingNo).filter(Boolean))].sort();
  const members = getDrawingTeamMembers();

  let rows = filterByStatusTab(baseList, state);
  rows = filterByProject(rows);
  rows = filterByDrawing(rows);
  rows = filterByColumnStatus(rows);
  rows = filterByAssignee(rows);
  rows = filterByDate(rows);
  rows = filterBySearch(rows);

  // sort: อัปเดตล่าสุดขึ้นบนสุด
  rows = [...rows].sort((a, b) => {
    const da = new Date(a.updatedAt || a.submittedAt || 0);
    const db = new Date(b.updatedAt || b.submittedAt || 0);
    return db - da;
  });
  ...
  view.innerHTML = `
    <section class="content-section track-workbook">
      ...
      <div class="track-filter-bar" style="flex-wrap:wrap;gap:8px;">
        ...
        <label class="track-select-filter">
          <span>Drawing</span>
          <select id="track-drawing-filter">
            <option value="">ทุก Drawing</option>
            ${drawingOptions.map((no) => `<option value="${escapeHtml(no)}" ${drawingFilter === no ? "selected" : ""}>${escapeHtml(no)}</option>`).join("")}
          </select>
        </label>
        ...
        <label class="track-select-filter">
          ...
          <select id="track-status-filter">
            ...
            ${Object.entries(STATUS_LABELS).map(([val, label]) => `<option value="${escapeHtml(val)}" ${columnStatusFilter === val ? "selected" : ""}>${escapeHtml(label)}</option>`).join("")}
          </select>
        </label>
      ...
      <div class="track-tab-groups">
        <div class="track-tab-group">
          ...
          <div class="track-tab-row">
            ...
            ${(()=>{
              const myEmail = getMyEmail(state);
              const awaitingItems = state.requests.filter((i) =>
                i.status === STATUS.DELIVERED &&
                String(i.requesterEmail || "").toLowerCase() === myEmail
              );
              const awaitingCount = awaitingItems.length;
              // หาจำนวนวันคงเหลือน้อยสุด (auto-approve ใน 3 วัน)
              const now = new Date();
              const minDays = awaitingItems.reduce((min, i) => {
                if (!i.reviewDeadline) return min;
                const wd = countWorkingDays(new Date(), new Date(i.reviewDeadline), holidaySet);
                return Math.min(min, wd);
              }, Infinity);
              const urgentBadge = awaitingCount > 0 && minDays <= 1
                ? `<span style="background:#EF4444;color:#fff;font-size:9px;font-weight:700;padding:1px 5px;border-radius:999px;margin-left:4px;">${minDays <= 0 ? "หมดเวลา" : minDays+" วันทำงาน"}</span>`
                : awaitingCount > 0 && minDays <= 2
                ? `<span style="background:#F59E0B;color:#fff;font-size:9px;font-weight:700;padding:1px 5px;border-radius:999px;margin-left:4px;">${minDays} วันทำงาน</span>`
                : "";
              return scopeStatusTabButton("mine", "awaiting", "📦 รอตรวจรับ", awaitingCount) + urgentBadge;
            })()}
          </div>
        </div>
      ...
      ${statusTab === "awaiting" ? `
        ${rows.length === 0
          ...
          : `<div style="margin-bottom:12px;padding:10px 14px;background:#FEF3C7;border-radius:8px;border-left:3px solid #F59E0B;font-size:12px;color:#92400E;">
              ⏰ <b>หมายเหตุ:</b> หากไม่ตรวจรับภายใน <b>3 วัน</b> ระบบจะ Auto-Approve อัตโนมัติ
            </div>
            <div class="admin-list">
              ${rows.map((item) => {
                const reviewDeadline = item.reviewDeadline ? new Date(item.reviewDeadline) : null;
                const wdLeft = reviewDeadline ? countWorkingDays(new Date(), reviewDeadline, holidaySet) : null;
                const dayBadge = wdLeft !== null
                  ? `<span style="display:inline-block;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;background:${wdLeft <= 0 ? "#FEE2E2" : wdLeft <= 1 ? "#FFEDD5" : "#DCFCE7"};color:${wdLeft <= 0 ? "#991B1B" : wdLeft <= 1 ? "#C2410C" : "#065F46"};">
                      ${wdLeft <= 0 ? "⚠️ หมดเวลาแล้ว" : `⏰ เหลือ ${wdLeft} วันทำงาน`}
                    </span>`
                  : "";
                // inject badge เข้าใน review card
                return renderReviewCard(item).replace(
                  '<span class="badge badge-delivered">📦 รอตรวจรับ</span>',
                  `<span class="badge badge-delivered">📦 รอตรวจรับ</span> ${dayBadge}`
                );
              }).join("")}
            </div>`
        }
        ...
        <table class="track-sheet">
          <thead>
            <tr>
              ...
              <th>ผู้ร้องขอ</th>
              <th>
                <select id="track-col-project-filter" class="track-col-filter-select">
                  <option value="">โครงการ (ทั้งหมด)</option>
                  ${projectOptions.map((name) => `<option value="${escapeHtml(name)}" ${projectFilter === name ? "selected" : ""}>${escapeHtml(name)}</option>`).join("")}
                </select>
              </th>
              <th>
                <select id="track-col-drawing-filter" class="track-col-filter-select">
                  <option value="">DRAWING (ทั้งหมด)</option>
                  ${drawingOptions.map((no) => `<option value="${escapeHtml(no)}" ${drawingFilter === no ? "selected" : ""}>${escapeHtml(no)}</option>`).join("")}
                </select>
              </th>
              <th>กำหนดส่ง</th>
              ...
              <th>
                <select id="track-col-status-filter" class="track-col-filter-select">
          ...
          <tbody>
            ${rows.length
              ? rows.map((item) => renderWorkbookRow(item, state, holidaySet)).join("")
              : `<tr><td colspan="8" class="track-empty">ไม่พบงานในหมวดนี้</td></tr>`}
          </tbody>
        </table>
      </div>
      `}
  ...
  bindReviewInteractions(view, state);
}

function scopeStatusTabButton(scope, status, label, count = null) {
  const isActive = scopeTab === scope && statusTab === status;
  const isAwaiting = status === "awaiting";
  const alertClass = isAwaiting && count > 0 ? "requester-tab-alert" : "";
  const badge = count > 0 ? `<span class="requester-tab-badge${isAwaiting ? "" : " requester-tab-badge-gray"}">${count}</span>` : "";
  return `<button class="filter-button ${isActive ? "is-active" : ""} ${alertClass}" data-scope="${scope}" data-status-tab="${status}" type="button">${label}${badge}</button>`;
}

function renderWorkbookRow(item, state, holidaySet = new Set()) {
  const due = dueSummary(item.dueDate, item.status, holidaySet);
  ...
  return `
    <tr>
      <td data-label="เหลือ/เกินกำหนด"><span class="due-chip ${due.className}">${escapeHtml(due.label)}</span></td>
      <td data-label="เลขที่คำร้อง"><button class="track-request-link" data-request-detail="${escapeHtml(item.requestNo)}" type="button">${escapeHtml(item.requestNo)}</button></td>
      <td data-label="ผู้ร้องขอ" style="min-width:190px;">
        <span style="font-weight:600;">${escapeHtml(item.requesterName || "—")}</span><br>
        <small style="color:#94A3B8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:210px;display:inline-block;vertical-align:top;" title="${escapeHtml(item.requesterEmail || "")}">${escapeHtml(item.requesterEmail || "")}</small>
        ${canTransferRequester(item, state) ? `
          <br><button type="button" class="small-button secondary-button"
            data-transfer-requester="${escapeHtml(item.requestNo)}"
            title="โอนคำร้องนี้ให้ผู้ร้องขอคนใหม่ (กรณีผู้ร้องขอเดิมลาออก/ย้ายงาน)"
            style="margin-top:5px;font-size:11px;padding:2px 8px;">👤 โอนผู้ร้องขอ</button>
        ` : ""}
      </td>
      <td data-label="โครงการ"><strong>${escapeHtml(item.projectName || "—")}</strong></td>
      <td data-label="Drawing"><strong>${escapeHtml(item.drawingNo || "—")}</strong><br><small>${escapeHtml(item.drawingName || "")}</small></td>
      <td data-label="กำหนดส่ง">${formatDateOnly(item.dueDate)}</td>
      <td data-label="โฟลเดอร์ / ลิงก์" style="white-space:nowrap;">
        ...
        ${item.dwgFileUrl ? `<a href="${escapeHtml(item.dwgFileUrl)}" target="_blank" rel="noopener noreferrer"
          class="mgr-file-btn dwg-btn" style="display:inline-flex;align-items:center;gap:4px;font-size:12px;padding:3px 8px;margin-top:4px;">
          📐 DWG
        </a>` : ""}
        ${item.pdfFileUrl ? `<a href="${escapeHtml(item.pdfFileUrl)}" target="_blank" rel="noopener noreferrer"
        ...
        ${parseNoteAndFiles(item.noteFromDrawing || "").otherFiles.map((f) => {
          const fname = f.name || "ไฟล์แนบ"; const shortName = fname.length > 18 ? fname.slice(0, 15) + "…" : fname;
          return `<a href="${escapeHtml(f.url)}" target="_blank" rel="noopener noreferrer"
            class="mgr-file-btn other-btn" title="${escapeHtml(fname)}"
            style="display:inline-flex;align-items:center;gap:4px;font-size:12px;padding:3px 8px;margin-top:4px;max-width:160px;">
            📎 <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(shortName)}</span>
          </a>`;
        }).join("")}
      ...
      <td data-label="สถานะ" style="vertical-align:middle;text-align:center;width:155px;">
        ${canEditStatus ? `
...
function bindWorkbookEvents(view, state) {
  ...
  bindSearchInput(view, "#track-search-input",
    (val) => { searchQuery = val; },
    () => renderWorkbookTrack(view, state));
  ...
  view.querySelector("#track-col-project-filter")?.addEventListener("change", (e) => {
    projectFilter = e.target.value;
    renderWorkbookTrack(view, state);
  }, opts);

  view.querySelector("#track-col-drawing-filter")?.addEventListener("change", (e) => {
    drawingFilter = e.target.value;
    renderWorkbookTrack(view, state);
  }, opts);

  view.querySelector("#track-col-status-filter")?.addEventListener("change", (e) => {
    columnStatusFilter = e.target.value;
    renderWorkbookTrack(view, state);
  }, opts);

  // ตัวกรองชุดเดียวกัน แต่อยู่ในแถบตัวกรองด้านนอก (มือถือมองเห็นเสมอ แม้หัวตารางจะถูกซ่อน)
  view.querySelector("#track-drawing-filter")?.addEventListener("change", (e) => {
    drawingFilter = e.target.value;
    renderWorkbookTrack(view, state);
  }, opts);

  view.querySelector("#track-status-filter")?.addEventListener("change", (e) => {
    columnStatusFilter = e.target.value;
    renderWorkbookTrack(view, state);
  }, opts);
  ...
  view.addEventListener("click", (event) => {
    ...
    const transferButton = event.target.closest("[data-transfer-requester]");
    if (transferButton) {
      const request = state.requests.find((item) => item.requestNo === transferButton.dataset.transferRequester);
      if (request) openTransferRequesterModal(view, state, request);
    }
  }, opts);
...
/**
 * โอนผู้ร้องขอ — ใช้เมื่อผู้ส่งคำร้องเดิมลาออก/ย้ายงาน
 * คำร้องเดิมไม่ขาดตอน ประวัติ Audit Log ยังครบ และผู้รับช่วงต่อจะเห็นงานใน "งานของฉัน" ทันที
 */
/**
 * Modal โอนผู้ร้องขอ — ใช้ร่วมกันทั้งหน้าติดตามงาน (track.js) และหน้ารับงาน/อนุมัติ (admin.js)
 * เพื่อให้ผู้ใช้เห็นฟอร์มเดียวกันทุกจุดในระบบ ไม่ว่าจะกดปุ่มจากหน้าไหนก็ตาม
 * @param {Function} [onSuccess] เรียกหลังโอนสำเร็จ เพื่อ re-render หน้าปัจจุบัน
 *   ถ้าไม่ส่งมา จะ re-render หน้าติดตามงานเป็นค่าเริ่มต้น (สำหรับผู้เรียกจาก track.js เอง)
 *   ผู้เรียกจากไฟล์อื่น (เช่น admin.js) ควรส่ง callback ของตัวเองมาเสมอ
 */
export function openTransferRequesterModal(view, state, request, onSuccess) {
  const candidates = getKnownRequesters(request.requesterEmail);
  const frequent   = candidates.filter((c) => c.count > 0);
  const others     = candidates.filter((c) => c.count === 0);
  ...
  const body = `
    <div class="assign-work-panel">
      ...
      <div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:8px;padding:10px 14px;margin:12px 0;font-size:12.5px;color:#1E3A8A;line-height:1.6;">
        👤 <b>ผู้ร้องขอปัจจุบัน:</b> ${escapeHtml(request.requesterName || "—")}
        <br><small style="color:#3B82F6;">${escapeHtml(request.requesterEmail || "")}</small>
        <br><br>ใช้เมื่อผู้ร้องขอเดิม<b>ลาออก / ย้ายงาน / ไม่สามารถตรวจรับงานได้</b>
        — คำร้องเดิมไม่ขาดตอน ประวัติทั้งหมดยังอยู่ครบ และผู้รับช่วงต่อจะเห็นงานนี้ใน "งานของฉัน" ทันที
      </div>
      ...
      <label class="field">
        ...
        <select id="tr-select">
          ...
          ${frequent.length ? `<optgroup label="เคยส่งคำร้องในระบบ">
            ${frequent.map((c) => `<option value="${escapeHtml(c.email)}" data-name="${escapeHtml(c.name)}">${escapeHtml(c.name)} (${c.count} คำร้อง)</option>`).join("")}
          </optgroup>` : ""}
          ${others.length ? `<optgroup label="ฝ่ายเขียนแบบ">
            ${others.map((c) => `<option value="${escapeHtml(c.email)}" data-name="${escapeHtml(c.name)}">${escapeHtml(c.name)}</option>`).join("")}
          </optgroup>` : ""}
          <option value="__custom__">➕ อื่นๆ — ระบุชื่อและอีเมลเอง</option>
        </select>
      </label>

      <div id="tr-custom" hidden style="margin-top:8px;display:flex;flex-direction:column;gap:8px;">
        <label class="field">
          <span>ชื่อผู้ร้องขอใหม่ <b class="req">*</b></span>
          <input id="tr-name" type="text" placeholder="ชื่อ-นามสกุล" />
        </label>
        <label class="field">
          <span>อีเมลผู้ร้องขอใหม่ <b class="req">*</b></span>
          <input id="tr-email" type="email" placeholder="name@primepower.co.th" />
        </label>
      </div>

      <label class="field" style="margin-top:8px;">
        <span>เหตุผลที่โอน <b class="req">*</b></span>
        <select id="tr-reason-select">
          <option value="">— เลือกเหตุผล —</option>
          <option value="ผู้ร้องขอเดิมลาออก">ผู้ร้องขอเดิมลาออก</option>
          <option value="ผู้ร้องขอเดิมย้ายหน่วยงาน">ผู้ร้องขอเดิมย้ายหน่วยงาน</option>
          <option value="เปลี่ยนผู้รับผิดชอบโครงการ">เปลี่ยนผู้รับผิดชอบโครงการ</option>
          <option value="ผู้ร้องขอเดิมลาระยะยาว">ผู้ร้องขอเดิมลาระยะยาว</option>
          <option value="อื่นๆ">อื่นๆ — ระบุเพิ่มเติม</option>
        </select>
        <input id="tr-reason-note" type="text" style="margin-top:8px;" placeholder="รายละเอียดเพิ่มเติม (ถ้ามี)..." />
      </label>

      <div style="margin-top:10px;padding:8px 12px;background:#FEF3C7;border-radius:8px;font-size:11.5px;color:#92400E;line-height:1.6;">
        📣 ระบบจะแจ้งเตือนทาง Teams ให้ <b>ผู้ร้องขอคนใหม่</b> และ <b>ผู้รับผิดชอบงาน</b> โดยอัตโนมัติ
      </div>
    </div>
  `;
  ...
  openModal({
    ...
    actions: [
      { label: "ยกเลิก", className: "secondary-button" },
      {
        ...
        onClick: async (close) => {
          const select = document.querySelector("#tr-select");
          const reasonSelect = document.querySelector("#tr-reason-select");
          const reasonNote   = document.querySelector("#tr-reason-note")?.value.trim() || "";

          let newRequester;
          if (select?.value === "__custom__") {
            const name  = document.querySelector("#tr-name")?.value.trim();
            const email = document.querySelector("#tr-email")?.value.trim();
            if (!name || !email) { showToast("กรุณากรอกชื่อและอีเมลผู้ร้องขอใหม่ให้ครบ", "warning"); return; }
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { showToast("รูปแบบอีเมลไม่ถูกต้อง", "warning"); return; }
            newRequester = { name, email };
          } else if (select?.value) {
            const opt = select.options[select.selectedIndex];
            newRequester = { name: opt.dataset.name || select.value, email: select.value };
          } else {
            showToast("กรุณาเลือกผู้ร้องขอคนใหม่", "warning");
            return;
          }

          if (String(newRequester.email).toLowerCase() === String(request.requesterEmail || "").toLowerCase()) {
            showToast("ผู้ร้องขอคนใหม่ซ้ำกับคนเดิม", "warning");
            return;
          }
          if (!reasonSelect?.value) { showToast("กรุณาเลือกเหตุผลที่โอน", "warning"); return; }

          const reason = reasonSelect.value === "อื่นๆ"
            ? (reasonNote || "อื่นๆ")
            : (reasonNote ? `${reasonSelect.value} — ${reasonNote}` : reasonSelect.value);
          ...
          try {
            ...
            // อัปเดต state ทันทีเพื่อให้ตารางแสดงค่าใหม่โดยไม่ต้องรีโหลด
            request.requesterName  = newRequester.name;
            request.requesterEmail = newRequester.email;
            showToast(`โอนคำร้อง ${request.requestNo} ให้ ${newRequester.name} แล้ว`, "success");
            close();
            (onSuccess || (() => rerenderTrack(view, state)))();
          } catch (error) {
            request.requesterName  = oldName;
            request.requesterEmail = oldEmail;
            showToast(`โอนไม่สำเร็จ: ${error.message}`, "error");
          }
        },
      },
    ],
  ...
  setTimeout(() => {
    const select = document.querySelector("#tr-select");
    const custom = document.querySelector("#tr-custom");
...
function openCancelOwnWorkModal(view, state, request) {
  const body = `
    ...
    <div style="margin-bottom:6px;">
          <label style="display:block;font-size:12px;font-weight:600;color:#991B1B;margin-bottom:4px;">
            ต้นเหตุการยกเลิก <b style="color:#EF4444;">*</b>
          </label>
          <select id="cancel-own-source" style="width:100%;padding:8px;border:1.5px solid #FCA5A5;background:#FEF2F2;border-radius:6px;font-size:13px;">
            <option value="">— เลือกต้นเหตุ —</option>
            ${CANCEL_SOURCES.map((s) => `<option value="${s.code}">${s.emoji} ${escapeHtml(s.label)}</option>`).join("")}
          </select>
          <div id="cancel-own-source-hint" style="margin-top:5px;font-size:10.5px;color:#B91C1C;line-height:1.5;">
            ℹ️ นับเข้า <b>KPI คุณภาพฝ่าย</b> เฉพาะรหัส 2.1 · นับเข้า <b>Wasted Cost</b> รหัส 2.1–2.4 เมื่อฝ่ายลงมือทำไปแล้ว
          </div>
        </div>
    <label class="field">
  ...
  setTimeout(() => {
    const sel = document.querySelector("#cancel-own-source");
    const hint = document.querySelector("#cancel-own-source-hint");
    sel?.addEventListener("change", () => {
      const meta = getCancelSource(sel.value);
      if (hint) hint.innerHTML = meta
        ? `${meta.emoji} <b>${escapeHtml(meta.short)}</b> — ${escapeHtml(meta.hint)}`
        : `ℹ️ นับเข้า <b>KPI คุณภาพฝ่าย</b> เฉพาะรหัส 2.1 · นับเข้า <b>Wasted Cost</b> รหัส 2.1–2.4 เมื่อฝ่ายลงมือทำไปแล้ว`;
    });
  }, 0);
    ...
    actions: [
      { label: "ปิด", className: "secondary-button" },
      {
        ...
        onClick: async (close) => {
          ...
          const source = document.querySelector("#cancel-own-source")?.value || "";
          if (!source) {
            showToast("กรุณาเลือกต้นเหตุการยกเลิก", "warning");
            return;
          }
          if (!reason) {
          ...
          try {
            await adminCancel(request, reason, source);
...
function openSendworkModal(view, state, request) {
  ...
  const body = `
    <form id="sendwork-form" class="delivery-form">
      ...
      <label class="delivery-file-drop" id="sendwork-file-drop">
        ...
        <small>DWG, PDF, รูปภาพ และเอกสารประกอบ</small>
      </label>
      <div id="sendwork-file-chips" style="display:none;flex-wrap:wrap;gap:6px;margin-top:8px;"></div>
      <label class="field">
  ...
  const chipsContainer = document.querySelector("#sendwork-file-chips");
  ...
  const renderChips = () => {
    ...
    chipsContainer.innerHTML = pendingFiles.map((f, i) => `
      <span style="display:inline-flex;align-items:center;gap:6px;background:#F0F9FF;border:1px solid #BFDBFE;border-radius:999px;padding:4px 6px 4px 12px;font-size:11.5px;color:#1E3A8A;max-width:280px;">
        <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${escapeHtml(f.name)}">${escapeHtml(f.name)}</span>
        <span style="color:#64748B;flex-shrink:0;">(${formatSize(f.size)})</span>
        <button type="button" data-sw-remove="${i}"
          style="flex-shrink:0;width:18px;height:18px;border-radius:50%;border:none;background:#FEE2E2;color:#EF4444;font-size:11px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;line-height:1;">✕</button>
      </span>
    `).join("") + `
      <button type="button" id="sendwork-add-more"
    ...
    chipsContainer.querySelectorAll("[data-sw-remove]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.preventDefault();
        const idx = parseInt(btn.dataset.swRemove, 10);
        pendingFiles = pendingFiles.filter((_, i) => i !== idx);
        renderChips();
      });
    });
    document.querySelector("#sendwork-add-more")?.addEventListener("click", (e) => {
      e.preventDefault();
      const tmp = document.createElement("input");
      tmp.type = "file";
      tmp.multiple = true;
      tmp.accept = ".dwg,.dxf,.pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.zip";
  ...
  const addFiles = (fileListLike) => {
    ...
    incoming.forEach((nf) => {
      const dup = pendingFiles.some((ef) => ef.name === nf.name && ef.size === nf.size);
...
// ══════════════════════════════════════════════════════════════
// REQUESTER VIEW — งานของฉัน / งานในฝ่าย (แยกตามประเภทของแบบ)
// "งานในฝ่าย" = โหมดดูอย่างเดียว: เปิด/ดาวน์โหลดไฟล์ได้ แต่แก้สถานะหรือไฟล์ไม่ได้
// ใช้ติดตามงานแทนเพื่อนร่วมทีมกรณีขาด/ลา/มาสาย
// ══════════════════════════════════════════════════════════════

// ลำดับประเภทของแบบที่ใช้แสดงเป็นปุ่มกรอง (ตรงกับ REQUEST_TYPES ในหน้าส่งคำร้อง)
...
// state ของหน้า requester
let reqScope       = "mine";   // "mine" | "dept"
let reqMyTab       = "";       // awaiting | inprogress | done | cancelled | all
let reqDeptType    = "";       // "" = ทุกประเภท, ไม่งั้นเป็นชื่อประเภทของแบบ
let reqDeptSearch  = "";
let reqDeptProject = "";
let reqDeptStatus  = "";

/** รวมรายชื่อประเภทของแบบที่มีจริงในระบบ เรียงตามลำดับมาตรฐานก่อน แล้วต่อด้วยประเภทอื่นที่พบ */
function getRequestTypeOptions(requests) {
  const found = new Set(requests.map((i) => i.requestType).filter(Boolean));
  const ordered = REQUEST_TYPE_ORDER.filter((t) => found.has(t));
  const extras  = [...found].filter((t) => !REQUEST_TYPE_ORDER.includes(t)).sort();
  return [...ordered, ...extras];
}

/** เซลล์ไฟล์แนบ — เปิด/ดาวน์โหลดได้อย่างเดียว (ไม่มีปุ่มแก้ไขใดๆ) */
function renderReadOnlyFileCell(item) {
  const folder     = getRequestFolderUrl(item.requestNo);
  const otherFiles = parseNoteAndFiles(item.noteFromDrawing || "").otherFiles;
  const btn = "display:inline-flex;align-items:center;gap:4px;font-size:11.5px;padding:3px 8px;margin:0 4px 4px 0;";
  return `
    <a href="${escapeHtml(folder)}" target="_blank" rel="noopener noreferrer" class="mgr-file-btn folder-btn" style="${btn}">📁 โฟลเดอร์</a>
    ${item.dwgFileUrl ? `<a href="${escapeHtml(item.dwgFileUrl)}" target="_blank" rel="noopener noreferrer" class="mgr-file-btn dwg-btn" style="${btn}">📐 DWG</a>` : ""}
    ${item.pdfFileUrl ? `<a href="${escapeHtml(item.pdfFileUrl)}" target="_blank" rel="noopener noreferrer" class="mgr-file-btn pdf-btn" style="${btn}">📄 PDF</a>` : ""}
    ${otherFiles.map((f) => {
      const fname = f.name || "ไฟล์แนบ"; const shortName = fname.length > 16 ? fname.slice(0, 13) + "…" : fname;
      return `<a href="${escapeHtml(f.url)}" target="_blank" rel="noopener noreferrer" class="mgr-file-btn other-btn" title="${escapeHtml(fname)}" style="${btn}max-width:150px;">📎 <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(shortName)}</span></a>`;
    }).join("")}
    ${item.dataLink ? `<a href="${escapeHtml(item.dataLink)}" target="_blank" rel="noopener noreferrer" class="mgr-file-btn ref-btn" style="${btn}">🔗 ลิงก์</a>` : ""}
  `;
}

/** แถวตารางในมุม "งานในฝ่าย" — อ่านอย่างเดียว */
function renderDeptRow(item) {
  return `
    <tr>
      <td data-label="เลขที่คำร้อง"><button class="track-request-link" data-request-detail="${escapeHtml(item.requestNo)}" type="button" title="คลิกเพื่อดูใบคำขอ">${escapeHtml(item.requestNo)}</button></td>
      <td data-label="ประเภทของแบบ"><span class="badge badge-pending" style="white-space:nowrap;">${escapeHtml(item.requestType || "—")}</span></td>
      <td data-label="โครงการ"><strong>${escapeHtml(item.projectName || "-")}</strong></td>
      <td data-label="Drawing"><strong>${escapeHtml(item.drawingNo || "-")}</strong><br><small>${escapeHtml(item.drawingName || "")}</small></td>
      <td data-label="ผู้ส่งคำร้อง">${escapeHtml(item.requesterName || "—")}</td>
      <td data-label="ผู้รับผิดชอบ">${escapeHtml(assigneeName(item) || "—")}</td>
      <td data-label="กำหนดส่ง" style="white-space:nowrap;">${formatDateOnly(item.dueDate)}</td>
      <td data-label="สถานะ"><span class="badge badge-${item.status}" style="white-space:nowrap;">${escapeHtml(STATUS_LABELS[item.status] || item.status)}</span></td>
      <td data-label="ไฟล์ / โฟลเดอร์" style="min-width:170px;">${renderReadOnlyFileCell(item)}</td>
    </tr>
  `;
}
...
function renderRequesterTrack(view, state) {
  ...
  // แท็บเริ่มต้นของ "งานของฉัน"
  if (!reqMyTab) reqMyTab = awaitingReview.length ? "awaiting" : "inprogress";

  // ── งานในฝ่ายทั้งหมด (ทุกคน) เรียงล่าสุดบนสุด ──
  const typeOptions = getRequestTypeOptions(state.requests);
  let deptRows = [...state.requests];
  if (reqDeptType)    deptRows = deptRows.filter((i) => (i.requestType || "") === reqDeptType);
  if (reqDeptProject) deptRows = deptRows.filter((i) => i.projectName === reqDeptProject);
  if (reqDeptStatus)  deptRows = deptRows.filter((i) => i.status === reqDeptStatus);
  if (reqDeptSearch.trim()) {
    const q = reqDeptSearch.toLowerCase().trim();
    deptRows = deptRows.filter((i) =>
      [i.requestNo, i.projectName, i.drawingNo, i.drawingName, i.requesterName, i.assignedToName]
        .some((v) => String(v || "").toLowerCase().includes(q))
    );
  }
  deptRows.sort((a, b) => new Date(b.submittedAt || 0) - new Date(a.submittedAt || 0));

  const deptProjects = [...new Set(state.requests.map((i) => i.projectName).filter(Boolean))].sort();
  const typeCount = (t) => state.requests.filter((i) => (i.requestType || "") === t).length;

  const myTabButton = (key, label, count = 0, alert = false) => `
    <button class="requester-tab ${reqScope === "mine" && reqMyTab === key ? "requester-tab-active" : ""} ${alert && count ? "requester-tab-alert" : ""}"
      data-req-scope="mine" data-req-tab="${key}" type="button">
      ${label}${count ? `<span class="requester-tab-badge${alert ? "" : " requester-tab-badge-gray"}">${count}</span>` : ""}
    </button>`;

  const deptTabButton = (typeValue, label, count) => `
    <button class="requester-tab ${reqScope === "dept" && reqDeptType === typeValue ? "requester-tab-active" : ""}"
      data-req-scope="dept" data-req-type="${escapeHtml(typeValue)}" type="button">
      ${escapeHtml(label)}${count ? `<span class="requester-tab-badge requester-tab-badge-gray">${count}</span>` : ""}
    </button>`;

  const myTable = (list, emptyMsg) => `
    <div class="track-table-wrap">
      <table class="track-sheet requester-track-table">
        ...
        <tbody>${list.length ? list.map((item) => renderRequesterRow(item)).join("") : `<tr><td colspan="6" class="track-empty">${emptyMsg}</td></tr>`}</tbody>
      </table>
    </div>`;
  ...
  view.innerHTML = `
    <section class="content-section requester-track">
      <div class="section-header">
        <h2>ติดตามงาน
          <span class="requester-result-count">${reqScope === "mine"
            ? `งานของฉัน — งานปัจจุบัน ${active.length} รายการ`
            : `งานในฝ่าย — ${deptRows.length} รายการ`}</span>
        </h2>
      </div>

      <div class="track-tab-groups track-tab-groups-stacked">
        <div class="track-tab-group">
          <div class="track-tab-group-label">งานของฉัน</div>
          <div class="requester-tab-row">
            ${myTabButton("awaiting", "📦 รอตรวจรับ", awaitingReview.length, true)}
            ${myTabButton("inprogress", "🔄 กำลังดำเนินการ", inProgress.length)}
            ${myTabButton("done", "✅ เสร็จสิ้น", done.length)}
            ${myTabButton("cancelled", "❌ ยกเลิก/ส่งกลับ", cancelled.length)}
            ${myTabButton("all", `📋 ทั้งหมด (${myItems.length})`)}
          </div>
        </div>

        <div class="track-tab-group track-tab-group-dept">
          <div class="track-tab-group-label">งานในฝ่าย — เลือกประเภทของแบบ</div>
          <div class="requester-tab-row">
            ${deptTabButton("", `📚 ทุกประเภท`, state.requests.length)}
            ${typeOptions.map((t) => deptTabButton(t, t, typeCount(t))).join("")}
          </div>
        </div>
      </div>
      ...
      ${reqScope === "mine" ? `
        ${reqMyTab === "awaiting" ? `
          ${awaitingReview.length
            ? `<div class="admin-list">${awaitingReview.map((item) => renderReviewCard(item)).join("")}</div>`
            : `<div class="track-empty" style="padding:40px;text-align:center;color:#94a3b8;">✅ ไม่มีรายการรอตรวจรับ</div>`}
        ` : ""}
        ${reqMyTab === "inprogress" ? myTable(inProgress, "ไม่มีงานที่กำลังดำเนินการ") : ""}
        ${reqMyTab === "done"       ? myTable(done, "ยังไม่มีงานเสร็จสิ้น") : ""}
        ${reqMyTab === "cancelled"  ? myTable(cancelled, "ไม่มีรายการยกเลิก") : ""}
        ${reqMyTab === "all"        ? myTable(myItems, "ยังไม่มีคำร้องของคุณ") : ""}
      ` : `
        <div style="margin-bottom:12px;padding:10px 14px;background:#EFF6FF;border-radius:8px;border-left:3px solid #3B82F6;font-size:12px;color:#1E3A8A;line-height:1.6;">
          👀 <b>โหมดดูอย่างเดียว</b> — ใช้ติดตามงานแทนเพื่อนร่วมทีมกรณีขาด/ลา/มาสาย
          <br>เปิดดูใบคำขอและ<b>ดาวน์โหลดไฟล์แนบได้</b> แต่<b>ไม่สามารถแก้ไขสถานะหรือแก้ไขไฟล์ได้</b>
        </div>

        <div class="track-filter-bar" style="flex-wrap:wrap;gap:8px;">
          <label class="track-search-field">
            <span>ค้นหางาน</span>
            <input id="req-dept-search" type="search"
              placeholder="ค้นหาเลขที่คำร้อง โครงการ Drawing ผู้ขอ ผู้รับผิดชอบ..."
              value="${escapeHtml(reqDeptSearch)}" />
          </label>
          <label class="track-select-filter">
            <span>โครงการ</span>
            <select id="req-dept-project">
              <option value="">ทุกโครงการ</option>
              ${deptProjects.map((n) => `<option value="${escapeHtml(n)}" ${reqDeptProject === n ? "selected" : ""}>${escapeHtml(n)}</option>`).join("")}
            </select>
          </label>
          <label class="track-select-filter">
            <span>สถานะ</span>
            <select id="req-dept-status">
              <option value="">ทุกสถานะ</option>
              ${Object.entries(STATUS_LABELS).map(([val, label]) => `<option value="${escapeHtml(val)}" ${reqDeptStatus === val ? "selected" : ""}>${escapeHtml(label)}</option>`).join("")}
            </select>
          </label>
          <button id="req-dept-clear" class="secondary-button" type="button">ล้าง</button>
        </div>
          ...
          <table class="track-sheet">
            <thead>
              <tr>
                ...
                <th>กำหนดส่ง</th>
                <th>สถานะ</th>
                <th>ไฟล์ / โฟลเดอร์ (ดาวน์โหลดได้)</th>
              </tr>
            </thead>
            <tbody>
              ${deptRows.length
                ? deptRows.map((item) => renderDeptRow(item)).join("")
                : `<tr><td colspan="9" class="track-empty">ไม่พบคำร้องในประเภทนี้</td></tr>`}
            </tbody>
          </table>
        </div>
      `}
  ...
  // ── สลับ scope / tab ── (ใช้ re-render เพื่อให้ state คงที่ทุกครั้ง)
  view.querySelectorAll("[data-req-scope]").forEach((btn) => {
    btn.addEventListener("click", () => {
      reqScope = btn.dataset.reqScope;
      if (reqScope === "mine") reqMyTab = btn.dataset.reqTab;
      else reqDeptType = btn.dataset.reqType || "";
      renderRequesterTrack(view, state);
    });
  });

  // ── ตัวกรองในมุมงานในฝ่าย ──
  bindSearchInput(view, "#req-dept-search",
    (val) => { reqDeptSearch = val; },
    () => renderRequesterTrack(view, state));
  view.querySelector("#req-dept-project")?.addEventListener("change", (e) => {
    reqDeptProject = e.target.value;
    renderRequesterTrack(view, state);
  });
  view.querySelector("#req-dept-status")?.addEventListener("change", (e) => {
    reqDeptStatus = e.target.value;
    renderRequesterTrack(view, state);
  });
  view.querySelector("#req-dept-clear")?.addEventListener("click", () => {
    reqDeptSearch = ""; reqDeptProject = ""; reqDeptStatus = "";
    renderRequesterTrack(view, state);
  });
    ...
    <article class="admin-card" id="review-card-${escapeHtml(item.requestNo)}">
      ...
      <div class="mgr-file-review-block">
        ...
        <div class="mgr-file-review-links">
          ...
          ${otherFiles.map((f) => {
            const fname = f.name || "ไฟล์แนบ"; const shortName = fname.length > 22 ? fname.slice(0, 19) + "…" : fname;
            return `<a href="${escapeHtml(f.url)}" target="_blank" rel="noopener noreferrer" class="mgr-file-btn other-btn" title="${escapeHtml(fname)}"><span>📎</span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:140px;">${escapeHtml(shortName)}</span></a>`;
          }).join("")}
      ...
      <div id="review-box-${escapeHtml(item.requestNo)}" class="admin-action-box" hidden>
        ...
        <!-- Dropdown สาเหตุ — แสดงเฉพาะตอน "ขอแก้ไข" -->
        <div id="review-source-wrap-${escapeHtml(item.requestNo)}" hidden style="margin-bottom:8px;">
          <label style="font-size:12px;font-weight:600;color:var(--text-secondary);display:block;margin-bottom:4px;">สาเหตุการแก้ไข <b style="color:#EF4444;">*</b></label>
          <select id="review-source-${escapeHtml(item.requestNo)}" style="width:100%;height:34px;padding:0 10px;border:1px solid var(--line);border-radius:8px;font-size:13px;background:var(--surface-1);">
            <option value="">-- เลือกสาเหตุ --</option>
            <option value="1.1">1.1 ความผิดพลาดจากฝ่ายเขียนแบบ</option>
            <option value="1.2">1.2 ความผิดพลาดจากผู้ส่งคำร้อง</option>
            <option value="1.3">1.3 ความผิดพลาดจากงานติดตั้งโครงการ</option>
            <option value="1.4">1.4 ความผิดพลาดจากลูกค้า</option>
          </select>
          <div id="review-source-note-${escapeHtml(item.requestNo)}" style="display:none;margin-top:6px;padding:7px 10px;background:#FEF2F2;border-radius:6px;border-left:3px solid #EF4444;font-size:11px;color:#991B1B;">
            ⚠️ เช่น เกิดจากคำผิด ใส่รายละเอียดไม่ครบตามคำขอ — หากฝ่ายเขียนแบบตรวจสอบแล้วไม่พบความสอดคล้อง จะยกเลิกคำขอทันที
          </div>
        </div>
        ...
        <div id="review-due-wrap-${escapeHtml(item.requestNo)}" hidden style="margin-top:12px;">
          <span style="font-size:12px;font-weight:600;color:var(--text-secondary);display:block;margin-bottom:4px;">
            📅 กำหนดส่งใหม่ <b style="color:#EF4444;">*</b>
          </span>
          <input id="review-due-${escapeHtml(item.requestNo)}" type="date"
            style="width:100%;border:1.5px solid #FCD34D;background:#FFFBEB;border-radius:8px;padding:8px;font-size:13px;" />
          <div id="review-due-warn-${escapeHtml(item.requestNo)}" style="display:none;"></div>
        </div>
...
function renderRequesterRow(item) {
  return `
    <tr>
      <td data-label="เลขที่คำร้อง"><button class="track-request-link" data-request-detail="${escapeHtml(item.requestNo)}" type="button">${escapeHtml(item.requestNo)}</button></td>
      <td data-label="โครงการ"><strong>${escapeHtml(item.projectName || "-")}</strong></td>
      <td data-label="Drawing"><strong>${escapeHtml(item.drawingNo || "-")}</strong><small>${escapeHtml(item.drawingName || "")}</small></td>
      <td data-label="กำหนดส่ง">${formatDateOnly(item.dueDate)}</td>
      <td data-label="สถานะ"><span class="badge badge-${item.status}">${escapeHtml(STATUS_LABELS[item.status] || item.status)}</span></td>
      <td data-label="ไฟล์งาน">${item.pdfFileUrl ? `<a class="track-file-link" href="${escapeHtml(item.pdfFileUrl)}" target="_blank" rel="noopener noreferrer">FILE</a>` : `<span class="track-no-file">-</span>`}</td>
    </tr>
  `;
}
...
// ── Review card handler (ใช้ร่วมทุก role: requester + manager/designer) ──
const _reviewInteractionMap = new WeakMap();
function bindReviewInteractions(view, state) {
  _reviewInteractionMap.get(view)?.abort();
  const ac = new AbortController();
  ...
  view.addEventListener("click", async (event) => {
    ...
    // ลบไฟล์ที่เลือก
    const rmBtn = event.target.closest("[data-evidence-remove]");
    if (rmBtn) {
    ...
    if (action === "toggle-revise" || action === "toggle-reject") {
      toggleReviewBox(view, requestNo, action === "toggle-revise" ? "revise" : "reject");
      return;
    }
    if (action === "close-box") {
    ...
    if (action === "confirm-box") {
      ...
      const source = view.querySelector(`#review-source-${safeId}`)?.value || "";
      const link   = view.querySelector(`#review-evidence-link-${safeId}`)?.value.trim() || "";

      const newDue = view.querySelector(`#review-due-${safeId}`)?.value || "";
      const files  = getReviewFiles(requestNo);

      if (mode === "revise") {
        // ── บังคับกรอกให้ครบ: สาเหตุ / รายละเอียด / ไฟล์แนบ / กำหนดส่งใหม่ ──
        if (!source)        { showToast("กรุณาเลือกสาเหตุการแก้ไข (1.1–1.4)", "warning"); return; }
        if (!note)          { showToast("กรุณาระบุรายละเอียดที่ต้องแก้ไข", "warning"); return; }
        if (!files.length)  { showToast("กรุณาแนบไฟล์ประกอบอย่างน้อย 1 ไฟล์", "warning"); return; }
        if (!newDue)        { showToast("กรุณากรอกกำหนดส่งใหม่", "warning"); return; }

        // ── เตือนถ้ากำหนดส่งใหม่เป็นวันเดียวกัน (เตือนเฉยๆ ยังไปต่อได้) ──
        const now = new Date();
        const todayKey = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}`;
        if (newDue === todayKey) {
          const proceed = await confirmSameDayDueDate(newDue);
          if (!proceed) return;
        }
      } else if (!note) {
        showToast("กรุณาระบุเหตุผล", "warning"); return;
      }

      button.disabled = true;
      button.classList.add("is-loading");
      try {
        await requesterReview(request, mode, note, source, { files, link, newDueDate: newDue });
        reviewEvidenceFiles.delete(requestNo);
        showToast(`${requestNo} ${mode === "revise" ? "ส่งขอแก้ไขแล้ว" : "Reject แล้ว — ส่งกลับ Designer"}`, "warning");
        rerenderTrack(view, state);
      } catch (error) {
        showToast(`เกิดข้อผิดพลาด: ${error.message}`, "error");
        button.disabled = false;
...
function rerenderTrack(view, state) {
  if (state.user?.role === "requester") renderRequesterTrack(view, state);
  else renderWorkbookTrack(view, state);
}
...
function toggleReviewBox(view, requestNo, mode) {
  ...
  const srcWrap    = view.querySelector(`#review-source-wrap-${safeId}`);
  const srcSelect  = view.querySelector(`#review-source-${safeId}`);
  const srcNote    = view.querySelector(`#review-source-note-${safeId}`);
  const evWrap     = view.querySelector(`#review-evidence-wrap-${safeId}`);
  const evLink     = view.querySelector(`#review-evidence-link-${safeId}`);
  const dueWrap    = view.querySelector(`#review-due-wrap-${safeId}`);
  const dueInput   = view.querySelector(`#review-due-${safeId}`);
  const dueWarn    = view.querySelector(`#review-due-warn-${safeId}`);
  ...
  if (srcNote) srcNote.style.display = "none";
  // เคลียร์หลักฐานที่ค้างไว้ทุกครั้งที่เปิดกล่องใหม่
  if (evLink) evLink.value = "";
  reviewEvidenceFiles.delete(requestNo);
  renderEvidenceList(view, requestNo);
  if (evWrap) evWrap.hidden = false; // แนบหลักฐานได้ทั้ง revise และ reject

  // รีเซ็ตช่องกำหนดส่งใหม่ทุกครั้งที่เปิดกล่อง
  if (dueInput) dueInput.value = "";
  if (dueWarn) { dueWarn.style.display = "none"; dueWarn.innerHTML = ""; }

  if (mode === "revise") {
    label.innerHTML = "✏️ <b>ขอแก้ไข</b> — เลือกสาเหตุ แนบไฟล์ และกำหนดส่งใหม่";
    note.placeholder = "ระบุจุดที่ต้องการให้แก้ไข...";
    box.className = "admin-action-box tone-warning";
    // แสดง dropdown สาเหตุ + ช่องกำหนดส่งใหม่
    if (srcWrap) srcWrap.hidden = false;
    if (dueWrap) dueWrap.hidden = false;
    if (dueInput && dueWarn) {
      const now = new Date();
      const todayKey = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}`;
      dueInput.min = todayKey;
      dueInput.onchange = () => {
        if (dueInput.value === todayKey) {
          dueWarn.style.display = "block";
          dueWarn.innerHTML = `<div style="margin-top:7px;padding:9px 12px;background:#FEF2F2;border:1.5px solid #DC2626;border-radius:8px;font-size:11.5px;color:#B91C1C;font-weight:600;line-height:1.6;">
            🚨 กำหนดส่งใหม่เป็น<b>วันนี้</b> — กรุณาเผื่อเวลาให้ฝ่ายเขียนแบบได้แก้ไขงาน<br>
            <span style="font-weight:400;">ยังยืนยันใช้วันนี้ได้ ระบบจะถามยืนยันอีกครั้งตอนกดส่ง</span>
          </div>`;
        } else {
          dueWarn.style.display = "none";
          dueWarn.innerHTML = "";
        }
      };
    }
    // แสดง note เตือนเมื่อเลือก 1.1
    if (srcSelect && srcNote) {
      srcSelect.onchange = () => {
        srcNote.style.display = srcSelect.value === "1.1" ? "block" : "none";
      };
    }
  } else {
    ...
    // ซ่อน dropdown สาเหตุ + ช่องกำหนดส่งใหม่ สำหรับ Reject
...
export function openRequestDetail(request) {
  openModal({
      ...
      <div class="request-detail-layout">
        <article class="request-detail-panel">
          ...
          <div class="request-detail-grid">
            ...
            ${detailItemLink("Location โครงการ", request.location)}
            ...
            ${(request.revisionSource || request.revisionReason || (request.revisionEvidence && request.revisionEvidence !== "[]")) ? `
              <div class="request-detail-item request-detail-revision" style="grid-column:1/-1;background:#FFF7ED;border:1px solid #FED7AA;border-radius:8px;padding:10px 14px;margin-top:2px;">
                <div style="font-size:11px;font-weight:700;color:#C2410C;margin-bottom:6px;">🔄 สาเหตุที่ส่งกลับแก้ไข</div>
                ${request.revisionSource ? (() => {
                  const srcMap = { "1.1":"🔴 1.1 ความผิดพลาดจากฝ่ายเขียนแบบ","1.2":"🟡 1.2 ความผิดพลาดจากผู้ส่งคำร้อง","1.3":"🟠 1.3 ความผิดพลาดจากงานติดตั้งโครงการ","1.4":"🔵 1.4 ความผิดพลาดจากลูกค้า" };
                  return `<div style="font-size:12px;font-weight:600;color:#92400E;margin-bottom:4px;">${escapeHtml(srcMap[request.revisionSource] || request.revisionSource)}</div>`;
                })() : ""}
                ${request.revisionReason ? `<div style="font-size:12px;color:#78350F;line-height:1.6;white-space:pre-wrap;">${escapeHtml(request.revisionReason)}</div>` : ""}
                ${(() => {
                  let ev = [];
                  try { ev = JSON.parse(request.revisionEvidence || "[]"); } catch { ev = []; }
                  if (!Array.isArray(ev) || !ev.length) return "";
                  return `<div style="margin-top:8px;font-size:11px;font-weight:700;color:#C2410C;">📎 หลักฐานแนบ</div>` +
                    ev.map((f) => `<div style="font-size:12px;margin-top:2px;"><a href="${escapeHtml(f.url)}" target="_blank" rel="noopener noreferrer" style="color:#2563EB;">${escapeHtml(f.name || f.url)}</a></div>`).join("");
                })()}
              </div>` : ""}
          ...
          <div class="request-detail-files">
            ...
            <div class="mgr-file-review-links" style="margin-top:8px;flex-wrap:wrap;gap:8px;display:flex;">
              ...
              ${parseNoteAndFiles(request.noteFromDrawing || "").otherFiles.map((f) => {
                const fname = f.name || "ไฟล์แนบ"; const shortName = fname.length > 28 ? fname.slice(0, 25) + "…" : fname;
                return `<a href="${escapeHtml(f.url)}" target="_blank" rel="noopener noreferrer"
                  class="mgr-file-btn other-btn"
                  title="${escapeHtml(fname)}"
                  style="display:inline-flex;align-items:center;gap:6px;padding:6px 12px;font-size:13px;max-width:220px;overflow:hidden;">
                  📎 <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(shortName)}</span>
                </a>`;
              }).join("")}
              ${request.dataLink ? `
...
function detailItemLink(label, url) {
  if (!url) return "";
  return `<div class="request-detail-item"><span>${escapeHtml(label)}</span><strong><a href="${escapeHtml(url)}" target="_blank" rel="noreferrer">เปิดแผนที่ 📍</a></strong></div>`;
}
...
function countWorkingDays(fromDate, toDate, holidaySet) {
  ...
  // นับรวมวันนี้ด้วย (ยังมีเวลาทำงานวันนี้อยู่)
  const step = end >= cur ? 1 : -1;
  while (cur.getTime() !== end.getTime()) {
    const dow = cur.getDay();
    if (dow !== 0 && dow !== 6 && !holidaySet.has(cur.toDateString())) count += step;
    cur.setDate(cur.getDate() + step);
  }
  // นับวันครบกำหนด (ยังส่งได้ในวันนั้น)
  const endDow = end.getDay();
  if (endDow !== 0 && endDow !== 6 && !holidaySet.has(end.toDateString())) count += step;
  return count;
}

function dueSummary(dueDate, status, holidaySet = new Set()) {
  if (CLOSED_STATUSES.includes(status)) return { label: "ปิดแล้ว", className: "done" };
  if (status === "delivered") return { label: "ส่งมอบแล้ว", className: "done" };
  if (!dueDate) return { label: "-", className: "neutral" };
  const today = startOfDay(new Date());
  const due   = startOfDay(new Date(dueDate));
  // วันนี้ตรงกับวันครบกำหนดพอดี — ไม่ต้องนับจำนวนวัน
  if (today.getTime() === due.getTime()) return { label: "ครบกำหนดวันนี้", className: "today" };
  const workDays = countWorkingDays(today, due, holidaySet);
  if (workDays < 0) return { label: `เกินกำหนด ${Math.abs(workDays)} วันทำงาน`, className: "overdue" };
  return { label: `เหลืออีก ${workDays} วันทำงาน`, className: "soon" };
}
...
// ══════════════════════════════════════════════════════════════
// Section "คำร้องที่ฉันส่ง" — สำหรับ Designer/Manager ที่ส่งคำร้องเอง
// แสดงใต้ Workbook section อัตโนมัติถ้ามีคำร้องที่ตัวเองส่ง
// ══════════════════════════════════════════════════════════════

function renderMyRequestsSection(view, state) {
  const myEmail = String(state.user?.email || "").toLowerCase();
  const myItems = state.requests.filter(
    (item) => String(item.requesterEmail || "").toLowerCase() === myEmail
  );

  // ไม่แสดง Section นี้ถ้าไม่มีคำร้องที่ตัวเองส่งเลย
  if (!myItems.length) return;

  const awaitingReview = myItems.filter((item) => item.status === STATUS.DELIVERED);
  const inProgress     = myItems.filter((item) => !CLOSED_STATUSES.includes(item.status) && item.status !== STATUS.DELIVERED);
  const done           = myItems.filter((item) => item.status === STATUS.DONE);
  const cancelled      = myItems.filter((item) => [STATUS.CANCELLED, STATUS.REJECTED].includes(item.status));

  // สร้าง wrapper div ต่อท้าย view
  const wrapper = document.createElement("div");
  wrapper.id = "my-requests-section";
  wrapper.style.cssText = "margin-top:2rem;";
  wrapper.innerHTML = `
    <section class="content-section requester-track" style="border-top:2px solid var(--line,#e2e8f0);padding-top:1.5rem;">
      <div class="section-header" style="display:flex;align-items:center;gap:12px;margin-bottom:1rem;">
        <h2 style="margin:0;">คำร้องที่ฉันส่ง</h2>
        ${awaitingReview.length ? `<span style="background:#F59E0B;color:#fff;font-size:11px;font-weight:700;padding:2px 10px;border-radius:999px;">${awaitingReview.length} รอตรวจรับ</span>` : ""}
        <span style="font-size:13px;color:var(--muted,#64748b);">ทั้งหมด ${myItems.length} รายการ</span>
      </div>

      <!-- Tab row -->
      <div class="requester-tab-row">
        <button class="requester-tab ${awaitingReview.length ? "requester-tab-alert" : ""}" data-my-tab="awaiting" type="button">
          📦 รอตรวจรับ
          ${awaitingReview.length ? `<span class="requester-tab-badge">${awaitingReview.length}</span>` : ""}
        </button>
        <button class="requester-tab" data-my-tab="inprogress" type="button">
          🔄 กำลังดำเนินการ
          ${inProgress.length ? `<span class="requester-tab-badge requester-tab-badge-gray">${inProgress.length}</span>` : ""}
        </button>
        <button class="requester-tab" data-my-tab="done" type="button">
          ✅ เสร็จสิ้น
          ${done.length ? `<span class="requester-tab-badge requester-tab-badge-gray">${done.length}</span>` : ""}
        </button>
        <button class="requester-tab" data-my-tab="cancelled" type="button">
          ❌ ยกเลิก/ส่งกลับ
          ${cancelled.length ? `<span class="requester-tab-badge requester-tab-badge-gray">${cancelled.length}</span>` : ""}
        </button>
        <button class="requester-tab" data-my-tab="all" type="button">
          📋 ทั้งหมด (${myItems.length})
        </button>
      </div>

      <!-- Panel: รอตรวจรับ -->
      <div id="my-panel-awaiting">
        ${awaitingReview.length
          ? `<div class="admin-list">${awaitingReview.map((item) => renderReviewCard(item)).join("")}</div>`
          : `<div class="track-empty" style="padding:40px;text-align:center;color:#94a3b8;">✅ ไม่มีรายการรอตรวจรับ</div>`}
      </div>

      <!-- Panel: กำลังดำเนินการ -->
      <div id="my-panel-inprogress" hidden>
        <div class="track-table-wrap">
          <table class="track-sheet requester-track-table">
            <thead><tr><th>เลขที่คำร้อง</th><th>โครงการ</th><th>Drawing</th><th>กำหนดส่ง</th><th>สถานะ</th><th>ไฟล์งาน</th></tr></thead>
      ...
      <!-- Panel: ยกเลิก/ส่งกลับ -->
      <div id="my-panel-cancelled" hidden>
        <div class="track-table-wrap">
          <table class="track-sheet requester-track-table">
            <thead><tr><th>เลขที่คำร้อง</th><th>โครงการ</th><th>Drawing</th><th>กำหนดส่ง</th><th>สถานะ</th><th>ไฟล์งาน</th></tr></thead>
      ...
      <!-- Panel: ทั้งหมด -->
      <div id="my-panel-all" hidden>
        <div class="track-table-wrap">
          <table class="track-sheet requester-track-table">
            <thead><tr><th>เลขที่คำร้อง</th><th>โครงการ</th><th>Drawing</th><th>กำหนดส่ง</th><th>สถานะ</th><th>ไฟล์งาน</th></tr></thead>
            <tbody>${myItems.map((item) => renderRequesterRow(item)).join("")}</tbody>
          </table>
        </div>
      </div>
    </section>
  ...
  // ── Tab click handler ──
  wrapper.querySelectorAll("[data-my-tab]").forEach((btn) => {
    btn.addEventListener("click", () => {
      wrapper.querySelectorAll("[data-my-tab]").forEach((b) => b.classList.remove("requester-tab-active"));
      btn.classList.add("requester-tab-active");
      ["awaiting","inprogress","done","cancelled","all"].forEach((name) => {
        const panel = wrapper.querySelector(`#my-panel-${name}`);
        if (panel) panel.hidden = btn.dataset.myTab !== name;
      });
    });
  });

  // เปิด tab รอตรวจรับก่อนถ้ามี
  const defaultTab = awaitingReview.length ? "awaiting" : "inprogress";
  wrapper.querySelector(`[data-my-tab="${defaultTab}"]`)?.click();

  // ── Event handler: ใช้ data-review-action เหมือน renderReviewCard เดิม ──
  const myAc = new AbortController();
  wrapper.addEventListener("click", async (event) => {
    if (myAc.signal.aborted) return;

    // เปิด popup รายละเอียด
    const detailBtn = event.target.closest("[data-request-detail]");
    if (detailBtn) {
      const req = state.requests.find((i) => i.requestNo === detailBtn.dataset.requestDetail);
    ...
    const deliveryBtn = event.target.closest("[data-delivery-form]");
    if (deliveryBtn) {
      const req = state.requests.find((i) => i.requestNo === deliveryBtn.dataset.deliveryForm);
      if (req) {
        if (req.deliveryFormUrl) window.open(req.deliveryFormUrl, "_blank", "noopener,noreferrer");
        else openDeliveryFormModal(view, state, req);
      }
      return;
    }

    // ปุ่ม data-review-action (จาก renderReviewCard)
    const button = event.target.closest("[data-review-action]");
    if (!button) return;
    ...
    if (reviewAction === "toggle-revise") {
      const box = wrapper.querySelector(`#review-box-${escapeHtml(requestNo)}`);
      if (box) { box.hidden = false; box.dataset.mode = "revise";
        const label = wrapper.querySelector(`#review-label-${escapeHtml(requestNo)}`);
        if (label) label.textContent = "✏️ ระบุสิ่งที่ต้องแก้ไข"; }
      return;
    }
    if (reviewAction === "toggle-reject") {
      const box = wrapper.querySelector(`#review-box-${escapeHtml(requestNo)}`);
      if (box) { box.hidden = false; box.dataset.mode = "reject";
        const label = wrapper.querySelector(`#review-label-${escapeHtml(requestNo)}`);
        if (label) label.textContent = "❌ ระบุเหตุผลที่ปฏิเสธ"; }
      return;
    }
    if (reviewAction === "close-box") {
      const box = wrapper.querySelector(`#review-box-${escapeHtml(requestNo)}`);
      if (box) box.hidden = true;
      return;
    }
    ...
    if (reviewAction === "approve") {
      ...
      try {
        ...
        myAc.abort();
        renderWorkbookTrack(view, state);
        renderMyRequestsSection(view, state);
      } catch (err) {
        showToast(`เกิดข้อผิดพลาด: ${err.message}`, "error");
        button.disabled = false;
        button.classList.remove("is-loading");
    ...
    if (reviewAction === "confirm-box") {
      const box  = wrapper.querySelector(`#review-box-${escapeHtml(requestNo)}`);
      const mode = box?.dataset.mode;
      const note = wrapper.querySelector(`#review-note-${escapeHtml(requestNo)}`)?.value.trim();
      if (!note) { showToast("กรุณาระบุเหตุผล", "warning"); return; }
      button.disabled = true;
      try {
        await requesterReview(request, mode, note);
        showToast(mode === "reject" ? `${requestNo} ปฏิเสธงานแล้ว` : `${requestNo} ขอแก้ไขแล้ว`, "success");
        myAc.abort();
        renderWorkbookTrack(view, state);
        renderMyRequestsSection(view, state);
      } catch (err) {
