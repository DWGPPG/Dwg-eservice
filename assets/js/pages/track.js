          ...
          <div class="track-tab-row">
            ...
            ${(()=>{
              ...
              const minDays = awaitingItems.reduce((min, i) => {
                if (!i.deliveredAt) return min;
                const deadline = new Date(new Date(i.deliveredAt).getTime() + 3 * 86400000);
                const diff = Math.ceil((deadline - now) / 86400000);
                return Math.min(min, diff);
              }, Infinity);
              const urgentBadge = awaitingCount > 0 && minDays <= 1
                ? `<span style="background:#EF4444;color:#fff;font-size:9px;font-weight:700;padding:1px 5px;border-radius:999px;margin-left:4px;">${minDays <= 0 ? "หมดเวลา" : minDays+" วัน"}</span>`
                : awaitingCount > 0 && minDays <= 2
                ? `<span style="background:#F59E0B;color:#fff;font-size:9px;font-weight:700;padding:1px 5px;border-radius:999px;margin-left:4px;">${minDays} วัน</span>`
                : "";
          ...
          : `<div style="margin-bottom:12px;padding:10px 14px;background:#FEF3C7;border-radius:8px;border-left:3px solid #F59E0B;font-size:12px;color:#92400E;">
            ...
            <div class="admin-list">
              ${rows.map((item) => {
                ...
                const deadline = deliveredAt ? new Date(deliveredAt.getTime() + 3 * 86400000) : null;
                const daysLeft = deadline ? Math.ceil((deadline - now) / 86400000) : null;
                const dayBadge = daysLeft !== null
                  ? `<span style="display:inline-block;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:700;background:${daysLeft <= 0 ? "#FEE2E2" : daysLeft <= 1 ? "#FFEDD5" : "#DCFCE7"};color:${daysLeft <= 0 ? "#991B1B" : daysLeft <= 1 ? "#C2410C" : "#065F46"};">
                      ${daysLeft <= 0 ? "⚠️ หมดเวลาแล้ว" : `⏰ เหลือ ${daysLeft} วัน`}
                    </span>`
                  : "";
...
function dueSummary(dueDate, status) {
  ...
  // ฝ่ายส่งมอบแล้ว — ไม่นับเกินกำหนด เพราะฝ่ายเขียนแบบทำเสร็จแล้ว รอผู้ร้องขอตรวจรับเท่านั้น
  if (status === "mgr_review" || status === "delivered") return { label: "ส่งมอบแล้ว", className: "done" };
