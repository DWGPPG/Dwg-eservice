      ...
      <td data-label="โฟลเดอร์ / ลิงก์" style="white-space:nowrap;">
        ...
        ${parseNoteAndFiles(item.noteFromDrawing || "").otherFiles.map((f) => {
          ...
          return `<a href="${escapeHtml(f.url)}" target="_blank" rel="noopener noreferrer"
            class="mgr-file-btn other-btn" title="${escapeHtml(f.name)}"
...
function renderReadOnlyFileCell(item) {
  ...
  return `
    ...
    ${otherFiles.map((f) => {
      const shortName = f.name.length > 16 ? f.name.slice(0, 13) + "…" : f.name;
      return `<a href="${escapeHtml(f.url)}" target="_blank" rel="noopener noreferrer" class="mgr-file-btn other-btn" title="${escapeHtml(f.name)}" style="${btn}max-width:150px;">📎 <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(shortName)}</span></a>`;
    }).join("")}
          ...
          <div class="request-detail-files">
            ...
            <div class="mgr-file-review-links" style="margin-top:8px;flex-wrap:wrap;gap:8px;display:flex;">
              ...
              ${parseNoteAndFiles(request.noteFromDrawing || "").otherFiles.map((f) => {
                const shortName = f.name.length > 28 ? f.name.slice(0, 25) + "…" : f.name;
                return `<a href="${escapeHtml(f.url)}" target="_blank" rel="noopener noreferrer"
