...
function renderDeptRow(item) {
  return `
    <tr>
      <td><button class="track-request-link" data-request-detail="${escapeHtml(item.requestNo)}" type="button" title="คลิกเพื่อดูใบคำขอ">${escapeHtml(item.requestNo)}</button></td>
      <td><span class="badge badge-pending" style="white-space:nowrap;">${escapeHtml(item.requestType || "—")}</span></td>
      <td><strong>${escapeHtml(item.projectName || "-")}</strong></td>
      <td><strong>${escapeHtml(item.drawingNo || "-")}</strong><br><small>${escapeHtml(item.drawingName || "")}</small></td>
      <td>${escapeHtml(item.requesterName || "—")}</td>
      <td>${escapeHtml(assigneeName(item) || "—")}</td>
      <td style="white-space:nowrap;">${formatDateOnly(item.dueDate)}</td>
      <td><span class="badge badge-${item.status}" style="white-space:nowrap;">${escapeHtml(STATUS_LABELS[item.status] || item.status)}</span></td>
      <td style="min-width:170px;">${renderReadOnlyFileCell(item)}</td>
    </tr>
  `;
}
