...
function bindRequesterReviewEvents(view, state) {
  ...
  view.addEventListener("click", async (event) => {
    ...
    if (action === "confirm-box") {
      ...
      // validate: ถ้าเป็น revise ต้องเลือกสาเหตุด้วย
      if (mode === "revise" && !source) {
        showToast("กรุณาเลือกสาเหตุการแก้ไข (1.1–1.4)", "warning");
        return;
      }
      if (!note) {
      ...
      button.classList.add("is-loading");
      try {
        await requesterReview(request, mode, note, source);
      ...
      <div class="request-detail-layout">
        <article class="request-detail-panel">
          ...
          <div class="request-detail-grid">
            ...
            ${(request.revisionSource || request.revisionReason) ? `
