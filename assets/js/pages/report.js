...
const WORK_DAYS_PER_MONTH = 20; // วันทำงาน/เดือน (หยุดเสาร์-อาทิตย์)
...
export function renderKpiDashboard(allRequests, team, period, person = null, customRange = null) {
  ...
  const overdueCostBaht    = Math.round(overdue * WORK_DAYS_PER_MONTH * WORK_HOURS_PER_DAY * OT_RATE_PER_HOUR);
  ...
  return `
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(155px,1fr));gap:10px;margin-bottom:16px;">
      ...
      <div id="dash-extra-cards" style="display:contents;"></div>
    </div>
    ...
    <div style="background:#FFF1F2;border:1.5px solid #FECACA;border-radius:14px;padding:20px;margin-bottom:14px;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px;margin-bottom:16px;">
        ...
        ${kpiCard("💸","ต้นทุนโอกาส",overdueCostBaht>0?`~${overdueCostBaht.toLocaleString()} ฿`:"—",`OT 94฿/ชม. (ค่าแรงขั้นต่ำวิชาชีพเฉพาะ*) × ${overdue} งานเกินกำหนด × 20 วัน/เดือน`,"#B91C1C")}
