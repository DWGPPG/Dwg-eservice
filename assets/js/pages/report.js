...
export function renderKpiDashboard(allRequests, team, period, person = null, customRange = null) {
  ...
  return `
    ...
    ${(() => {
      const revRows = rows.filter((r) => r.isRevision || (r.reviseNumber && r.reviseNumber !== "0" && r.reviseNumber !== ""));
      if (!revRows.length) return "";

      // ── LEFT: Top 10 Drawing ที่แก้บ่อยสุด (จัดกลุ่มตาม drawingNo) ──
      const drawingMap = {};
      revRows.forEach((r) => {
        const key = r.drawingNo || r.requestNo;
        if (!drawingMap[key]) drawingMap[key] = { drawingNo: r.drawingNo || "—", drawingName: r.drawingName || "—", projectName: r.projectName || "—", count: 0, reasons: [] };
        // ── นับจำนวนครั้งแก้ไข รองรับทั้งตัวเลข (1,2,3) และตัวอักษร (A,B,C) ──
      const parseRev = (v) => {
        if (!v || v === "0") return 0;
        const s = String(v).trim().toUpperCase();
        // ตัวอักษรเดี่ยว A-Z → 1-26
        if (/^[A-Z]$/.test(s)) return s.charCodeAt(0) - 64;
        // ดึงตัวเลขออกจาก "R1", "Rev2", "1", "02" ฯลฯ
        const m = s.match(/\d+/);
        return m ? parseInt(m[0], 10) : 1;
      };
      drawingMap[key].count += Math.max(1, parseRev(r.reviseNumber));
        if (r.revisionReason) drawingMap[key].reasons.push(r.revisionReason);
      });
      ...
      // ── RIGHT: Top 10 สาเหตุ (unique free text รายการ) ──
      const reasonList = revRows.filter((r) => r.revisionReason && r.revisionReason.trim()).sort((a, b) => {
        const ca = parseInt(b.reviseNumber || "1", 10) || 1;
        const cb = parseInt(a.reviseNumber || "1", 10) || 1;
        return ca - cb;
      }).slice(0, 10);
      ...
      <div style="background:#FFFBEB;border:1.5px solid #FDE68A;border-radius:14px;padding:20px;margin-bottom:14px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                  ...
                  ${top10Drawings.map((d, i) => `
                    <tr style="background:${i%2===0?"#fff":"#FFFBEB"};border-bottom:1px solid #FEF3C7;">
                      ...
                      <td style="padding:7px 10px;">
                        <div style="font-size:12px;font-weight:600;color:#1E293B;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${escapeHtml(d.drawingNo)}">${escapeHtml(d.drawingNo)}</div>
                        <div style="font-size:10px;color:#64748B;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${escapeHtml(d.drawingName)}">${escapeHtml(d.drawingName)}</div>
          ...
          <!-- RIGHT: สาเหตุการแก้ไข grouped by revisionSource -->
          <div style="background:#fff;border-radius:10px;border:1px solid #FDE68A;overflow:hidden;">
            <div style="padding:10px 14px;background:#FEF3C7;font-size:12px;font-weight:700;color:#92400E;border-bottom:1px solid #FDE68A;">
              📝 สาเหตุการแก้ไข — จัดกลุ่มตามประเภท
            </div>
            <div style="max-height:340px;overflow-y:auto;">
              ${(() => {
                const srcRows = revRows.filter((r) => r.revisionSource || r.revisionReason);
                if (!srcRows.length) return `<div style="padding:30px;text-align:center;color:#94A3B8;font-size:13px;">✅ ไม่มีสาเหตุที่บันทึกไว้</div>`;
                // จัดกลุ่มตาม revisionSource
                const groups = {};
                srcRows.forEach((r) => {
                  const src = r.revisionSource || "ไม่ระบุ";
                  if (!groups[src]) groups[src] = { src, label: sourceLabel[src] || `⚪ ${src}`, items: [] };
                  groups[src].items.push(r);
                });
                const groupOrder = ["1.1","1.2","1.3","1.4","ไม่ระบุ"];
                const sorted = groupOrder.filter((k) => groups[k]).map((k) => groups[k]);
                return sorted.map((g) => `
                  <div style="padding:8px 12px;background:#FEF3C7;border-bottom:1.5px solid #FDE68A;">
                    <span style="font-size:12px;font-weight:700;color:#92400E;">${escapeHtml(g.label)}</span>
                    <span style="font-size:11px;color:#B45309;margin-left:6px;">${g.items.length} รายการ</span>
                  </div>
                  ${g.items.map((r, i) => `
                    <div style="padding:7px 12px 7px 20px;border-bottom:1px solid #FEF3C7;background:${i%2===0?"#fff":"#FFFBEB"};">
                      <div style="font-size:11px;font-weight:600;color:#1E293B;">${escapeHtml(r.drawingNo||"—")} <span style="color:#64748B;font-weight:400;">· ${escapeHtml(r.projectName||"")}</span></div>
                      ${r.revisionReason ? `<div style="font-size:11px;color:#475569;margin-top:2px;line-height:1.4;">${escapeHtml(r.revisionReason)}</div>` : ""}
                      ${r.reviseNumber && r.reviseNumber !== "0" ? `<span style="font-size:10px;background:#FEE2E2;border-radius:4px;padding:1px 5px;color:#EF4444;margin-top:2px;display:inline-block;">Rev ${escapeHtml(r.reviseNumber)}</span>` : ""}
                    </div>
                  `).join("")}
                `).join("");
              })()}
