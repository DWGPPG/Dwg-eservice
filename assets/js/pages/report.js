...
export function renderKpiDashboard(allRequests, team, period, person = null, customRange = null) {
  ...
  const overdueRows = rows.filter((r) => !CLOSED_STATUSES.includes(r.status) && r.dueDate && new Date(r.dueDate) < todayStart);
...
function renderExecutiveReport(rows, options) {
  ...
  const todayStartForOverdue = new Date(); todayStartForOverdue.setHours(0,0,0,0);
  const overdueRows = rows.filter((row) => !CLOSED_STATUSES.includes(row.status) && row.dueDate && new Date(row.dueDate) < todayStartForOverdue).sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate));
...
function reportStatus(row) {
  const todayStartRS = new Date(); todayStartRS.setHours(0,0,0,0);
  const overdue = !CLOSED_STATUSES.includes(row.status) && row.dueDate && new Date(row.dueDate) < todayStartRS;
