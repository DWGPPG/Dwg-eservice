...
export function renderKpiDashboard(allRequests, team, period, person = null, customRange = null, holidays = []) {
  ...
  const overdueRows = rows.filter((r) => {
    ...
    if (r.status === STATUS.DELIVERED) return false; // ฝ่ายส่งมอบแล้ว ไม่เกินกำหนด
  ...
  const overdueDurations = overdueRows.map((r) => Math.max(0, countWD(new Date(r.dueDate), new Date())));
  ...
  const memberOverdue = team.map((u) => {
    ...
    return { name: u.name, overdue: mine.length, total: total_m,
      avgDays: mine.length ? Math.round(mine.reduce((s, r) => s + Math.max(0, countWD(new Date(r.dueDate), new Date())), 0) / mine.length) : 0 };
