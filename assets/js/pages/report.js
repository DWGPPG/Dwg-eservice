  ...
  return `
    ...
    ${(() => {
      ...
      revRows.forEach((r) => {
        ...
        const rev = parseInt(r.reviseNumber || "1", 10);
        drawingMap[key].count += isNaN(rev) ? 1 : Math.max(1, rev);
      ...
      <div style="background:#FFFBEB;border:1.5px solid #FDE68A;border-radius:14px;padding:20px;margin-bottom:14px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
          ...
          <!-- RIGHT: Top 10 สาเหตุ (free text) -->
          <div style="background:#fff;border-radius:10px;border:1px solid #FDE68A;overflow:hidden;">
            <div style="padding:10px 14px;background:#FEF3C7;font-size:12px;font-weight:700;color:#92400E;border-bottom:1px solid #FDE68A;">
              📝 สาเหตุการแก้ไข (Top ${reasonList.length}) — เรียงตาม Rev สูงสุด
            </div>
            <div style="max-height:340px;overflow-y:auto;">
              ${reasonList.map((r, i) => `
                <div style="padding:8px 12px;border-bottom:1px solid #FEF3C7;background:${i%2===0?"#fff":"#FFFBEB"};">
                  <div style="display:flex;align-items:flex-start;gap:8px;">
                    <span style="flex-shrink:0;font-weight:700;color:#D97706;font-size:12px;min-width:18px;">${i+1}</span>
                    <div style="flex:1;min-width:0;">
                      <div style="font-size:11px;color:#1E293B;line-height:1.5;">${escapeHtml(r.revisionReason)}</div>
                      <div style="margin-top:3px;display:flex;flex-wrap:wrap;gap:4px;align-items:center;">
                        <span style="font-size:10px;color:#64748B;">${escapeHtml(r.drawingNo||"—")}</span>
                        ${r.revisionSource ? `<span style="font-size:10px;background:#FEF3C7;border-radius:4px;padding:1px 5px;color:#92400E;">${escapeHtml(sourceLabel[r.revisionSource]||r.revisionSource)}</span>` : ""}
                        ${r.reviseNumber && r.reviseNumber !== "0" ? `<span style="font-size:10px;background:#FEE2E2;border-radius:4px;padding:1px 5px;color:#EF4444;">Rev ${escapeHtml(r.reviseNumber)}</span>` : ""}
                      </div>
                    </div>
                  </div>
                </div>
              `).join("")}
              ${!reasonList.length ? `<div style="padding:30px;text-align:center;color:#94A3B8;font-size:13px;">✅ ไม่มีสาเหตุที่บันทึกไว้</div>` : ""}
            </div>
          </div>
