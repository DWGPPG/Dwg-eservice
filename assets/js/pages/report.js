import { CLOSED_STATUSES, STATUS, STATUS_LABELS } from "../../../config/schema.js";
...
export function renderKpiDashboard(allRequests, team, period, person = null, customRange = null, holidays = []) {
  ...
  const teamForCancel = (team && team.length ? team : getDrawingTeamMembers()) || [];
  const drawingTeamEmails = new Set(
    teamForCancel.map((m) => String(m.email || "").toLowerCase()).filter(Boolean)
  );
  ...
  const cancelledByDrawingTeam = (r) => {
    const email = String(r.cancelledByEmail || "").toLowerCase();
    if (!email) return true; // ข้อมูลเก่า — ในระบบมีแต่ฝ่ายเขียนแบบที่กดยกเลิกได้อยู่แล้ว
    return drawingTeamEmails.has(email);
  };

  const allCancelledRows  = rows.filter((r) => r.status === STATUS.CANCELLED);
  // ยกเลิกก่อนเริ่มงาน — แสดงให้ดูได้ แต่ไม่กระทบ KPI
  ...
  // ยกเลิกโดยคนนอกฝ่ายเขียนแบบ — ไม่นับเข้า KPI เช่นกัน
  const outsideCancelCount = allCancelledRows.filter(
    (r) => workStartedAtCancel(r) && !cancelledByDrawingTeam(r)
  ).length;
  // ตีกลับ / ส่งกลับผู้ร้องขอ — ไม่ใช่การยกเลิกงาน
  const rejectedBackCount = rows.filter((r) => r.status === STATUS.REJECTED).length;

  const cancelRows   = allCancelledRows.filter((r) => workStartedAtCancel(r) && cancelledByDrawingTeam(r));
  ...
  return `
    ...
    <!-- Row 3.5: TOP 10 งานเกินกำหนด (อัปเดตตามจริง — เสร็จแล้วหลุดออก) -->
    ${(() => {
      if (!currentOverdueRows.length) return `
        <div style="background:#F0FDF4;border:1.5px solid #BBF7D0;border-radius:14px;padding:18px 20px;margin-bottom:14px;">
          <div style="font-size:14px;font-weight:700;color:#166534;">⏰ Top 10 Drawing ที่เกินกำหนด</div>
          <div style="text-align:center;padding:20px 0;color:#16A34A;font-size:13px;">✅ ไม่มีงานค้างเกินกำหนดในขณะนี้</div>
        </div>`;

      // จัดกลุ่มตาม Drawing (โครงการ + Drawing No.) เพื่อคำนวณ "ล่าช้าเฉลี่ย" ได้
      const map = new Map();
      currentOverdueRows.forEach((r) => {
        const key = `${r.projectName || "—"}||${r.drawingNo || r.requestNo}`;
        if (!map.has(key)) {
          map.set(key, {
            drawingNo: r.drawingNo || "—", drawingName: r.drawingName || "—",
            projectName: r.projectName || "—", items: [],
          });
        }
      ...
      const list = [...map.values()].map((g) => {
        ...
        return { ...g, count: g.items.length, avgDays: avg, maxDays: Math.max(...days) };
      }).sort((a, b) => (b.avgDays - a.avgDays) || (b.count - a.count)).slice(0, 10);

      const maxAvg = Math.max(1, ...list.map((x) => x.avgDays));
      const overallAvg = avgOverdueDaysNow;
      ...
      <div style="background:#FFF7ED;border:1.5px solid #FED7AA;border-radius:14px;padding:20px;margin-bottom:14px;">
        <div style="font-size:14px;font-weight:700;color:#9A3412;margin-bottom:6px;">
          ⏰ Top 10 Drawing ที่เกินกำหนด
          <span style="font-size:11px;font-weight:400;color:#C2410C;margin-left:8px;">ค้างอยู่ ${currentOverdueRows.length} รายการ · ล่าช้าเฉลี่ยรวม ${overallAvg} วันทำงาน</span>
        </div>
        <div style="margin-bottom:12px;padding:8px 12px;background:#FFEDD5;border-radius:8px;font-size:10.5px;color:#9A3412;line-height:1.6;">
          📌 แสดงเฉพาะงานที่<b>ยังค้างจริง</b> — เมื่อฝ่ายเขียนแบบอัปเดตว่า<b>เสร็จสิ้น</b>แล้ว รายการจะถูกตัดออกจากตารางนี้ทันที
        </div>
        <table style="width:100%;border-collapse:collapse;font-size:12px;background:#fff;border-radius:8px;overflow:hidden;">
          <thead>
            <tr style="background:#FFEDD5;">
              <th style="padding:7px 8px;text-align:center;color:#9A3412;font-weight:700;width:38px;">#</th>
              <th style="padding:7px 8px;text-align:left;color:#9A3412;font-weight:700;">Drawing</th>
              <th style="padding:7px 8px;text-align:left;color:#9A3412;font-weight:700;">โครงการ</th>
              <th style="padding:7px 8px;text-align:center;color:#9A3412;font-weight:700;">รายการ</th>
              <th style="padding:7px 8px;text-align:center;color:#9A3412;font-weight:700;">ล่าช้าเฉลี่ย</th>
              <th style="padding:7px 8px;text-align:left;color:#9A3412;font-weight:700;">ผู้รับผิดชอบ</th>
              <th style="padding:7px 8px;text-align:left;color:#9A3412;font-weight:700;width:90px;">ระดับ</th>
            </tr>
          </thead>
          <tbody>
            ${list.map((g, i) => {
              const owners = [...new Set(g.items.map((r) => shortPersonName(r.assignedToName || r.assignedToEmail || "—")))].join(", ");
              const refs   = g.items.map((r) => r.requestNo).join(", ");
              const tone   = g.avgDays > 7 ? "#DC2626" : g.avgDays > 3 ? "#EA580C" : "#F59E0B";
              return `
              <tr style="background:${i%2===0?"#FFFBF5":"#fff"};border-bottom:1px solid #FEF3C7;">
                <td style="padding:7px 8px;text-align:center;font-weight:800;color:${i<3?"#DC2626":"#94A3B8"};">${i+1}</td>
                <td style="padding:7px 8px;">
                  <b style="color:#1E293B;">${escapeHtml(g.drawingNo)}</b><br>
                  <small style="color:#94A3B8;">${escapeHtml(g.drawingName)}</small><br>
                  <small style="color:#CBD5E1;font-size:9.5px;">${escapeHtml(refs)}</small>
                </td>
                <td style="padding:7px 8px;color:#475569;">${escapeHtml(g.projectName)}</td>
                <td style="padding:7px 8px;text-align:center;font-weight:700;color:#7C3AED;">${g.count}</td>
                <td style="padding:7px 8px;text-align:center;font-weight:800;color:${tone};">${g.avgDays} วัน${g.count>1?`<br><small style="color:#94A3B8;font-weight:400;">สูงสุด ${g.maxDays} วัน</small>`:""}</td>
                <td style="padding:7px 8px;color:#475569;">${escapeHtml(owners)}</td>
                <td style="padding:7px 8px;">
                  <div style="background:#F1F5F9;border-radius:4px;height:8px;overflow:hidden;">
                    <div style="background:${tone};height:100%;width:${Math.round(g.avgDays/maxAvg*100)}%;border-radius:4px;"></div>
                  </div>
                </td>
              </tr>`;
            }).join("")}
    ...
    <!-- Row 3.6: TOP 10 รายการยกเลิกล่าสุด (เก็บถาวรเป็น KPI) -->
    ${(() => {
      const cancelledAll = rows.filter((r) => r.status === STATUS.CANCELLED);
      if (!cancelledAll.length) return `
        <div style="background:#F0FDF4;border:1.5px solid #BBF7D0;border-radius:14px;padding:18px 20px;margin-bottom:14px;">
          <div style="font-size:14px;font-weight:700;color:#166534;">🚫 Top 10 Drawing ที่มีรายการยกเลิก</div>
          <div style="text-align:center;padding:20px 0;color:#16A34A;font-size:13px;">✅ ไม่มีรายการยกเลิกในช่วงนี้</div>
        </div>`;

      // เรียง "ล่าสุดอยู่อันดับ 1" แล้วรันลงไปเรื่อยๆ — รายการใหม่เข้ามาจะดันอันเก่าลง
      const sorted = [...cancelledAll].sort((a, b) => {
        const ta = new Date(a.cancelledAt || a.reviewedAt || a.submittedAt || 0).getTime() || 0;
        const tb = new Date(b.cancelledAt || b.reviewedAt || b.submittedAt || 0).getTime() || 0;
        return tb - ta;
      }).slice(0, 10);

      const srcMap = { "1.1":"🔴 1.1 ฝ่ายเขียนแบบ","1.2":"🟡 1.2 ผู้ส่งคำร้อง","1.3":"🟠 1.3 งานติดตั้ง","1.4":"🔵 1.4 ลูกค้า" };
      const stageLabel = (r) => {
        if (!r.cancelledStage) return r.approvedLv2At ? "หลังเริ่มงาน" : "—";
        return STATUS_LABELS[r.cancelledStage] || r.cancelledStage;
      };

      return `
      <div style="background:#FAF5FF;border:1.5px solid #E9D5FF;border-radius:14px;padding:20px;margin-bottom:14px;">
        <div style="font-size:14px;font-weight:700;color:#6B21A8;margin-bottom:6px;">
          🚫 Top 10 Drawing ที่มีรายการยกเลิก
        ...
        <div style="margin-bottom:12px;padding:8px 12px;background:#F3E8FF;border-radius:8px;font-size:10.5px;color:#6B21A8;line-height:1.6;">
          📌 <b>เก็บถาวรเป็น KPI</b> — ถึงแม้ Drawing นั้นจะถูกส่งคำร้องใหม่และทำเสร็จไปแล้ว รายการยกเลิกจะยังคงแสดงอยู่
          <br>🔄 เรียง<b>รายการล่าสุดเป็นอันดับ 1</b> เมื่อมีรายการยกเลิกใหม่เข้ามาจะดันอันดับเดิมลงไปเรื่อยๆ
        </div>
        <table style="width:100%;border-collapse:collapse;font-size:12px;background:#fff;border-radius:8px;overflow:hidden;">
          <thead>
            <tr style="background:#F3E8FF;">
              <th style="padding:7px 8px;text-align:center;color:#6B21A8;font-weight:700;width:38px;">#</th>
              <th style="padding:7px 8px;text-align:left;color:#6B21A8;font-weight:700;">วันที่ยกเลิก</th>
              <th style="padding:7px 8px;text-align:left;color:#6B21A8;font-weight:700;">Drawing</th>
              <th style="padding:7px 8px;text-align:left;color:#6B21A8;font-weight:700;">โครงการ</th>
              <th style="padding:7px 8px;text-align:left;color:#6B21A8;font-weight:700;">ยกเลิกตอน</th>
              <th style="padding:7px 8px;text-align:left;color:#6B21A8;font-weight:700;">สาเหตุ / เหตุผล</th>
              <th style="padding:7px 8px;text-align:center;color:#6B21A8;font-weight:700;width:74px;">KPI</th>
            </tr>
          </thead>
          <tbody>
            ${sorted.map((r, i) => {
              ...
              return `
              <tr style="background:${i%2===0?"#FDFAFF":"#fff"};border-bottom:1px solid #F3E8FF;">
                <td style="padding:7px 8px;text-align:center;font-weight:800;color:${i===0?"#7C3AED":"#94A3B8"};">${i+1}</td>
                <td style="padding:7px 8px;color:#475569;white-space:nowrap;">
                  ${when ? escapeHtml(new Date(when).toLocaleDateString("th-TH",{year:"2-digit",month:"short",day:"numeric"})) : "—"}
                  ${i===0?`<br><small style="color:#7C3AED;font-weight:700;">ล่าสุด</small>`:""}
                </td>
                <td style="padding:7px 8px;">
                  <b style="color:#1E293B;">${escapeHtml(r.drawingNo || "—")}</b><br>
                  <small style="color:#94A3B8;">${escapeHtml(r.drawingName || "—")}</small><br>
                  <small style="color:#CBD5E1;font-size:9.5px;">${escapeHtml(r.requestNo || "")}</small>
                </td>
                <td style="padding:7px 8px;color:#475569;">${escapeHtml(r.projectName || "—")}</td>
                <td style="padding:7px 8px;color:#475569;white-space:nowrap;">${escapeHtml(stageLabel(r))}</td>
                <td style="padding:7px 8px;color:#475569;">
                  ${r.revisionSource ? `<div style="font-size:10.5px;font-weight:600;">${escapeHtml(srcMap[r.revisionSource] || r.revisionSource)}</div>` : ""}
                  <div style="font-size:10.5px;color:#94A3B8;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;" title="${escapeHtml(r.rejectReason || "")}">${escapeHtml(r.rejectReason || "—")}</div>
                  ${r.cancelledByName ? `<small style="color:#CBD5E1;font-size:9.5px;">โดย ${escapeHtml(shortPersonName(r.cancelledByName))}</small>` : ""}
                </td>
                <td style="padding:7px 8px;text-align:center;">
                  ${inKpi
                    ? `<span style="display:inline-block;padding:2px 7px;border-radius:999px;background:#FEE2E2;color:#991B1B;font-size:9.5px;font-weight:700;">นับ</span>`
                    : `<span style="display:inline-block;padding:2px 7px;border-radius:999px;background:#F1F5F9;color:#94A3B8;font-size:9.5px;font-weight:700;">ไม่นับ</span>`}
                </td>
              </tr>`;
            }).join("")}
          </tbody>
    ...
    <div style="background:#F5F3FF;border:1.5px solid #DDD6FE;border-radius:14px;padding:20px;margin-bottom:14px;">
      ...
      <div style="margin-bottom:14px;padding:9px 12px;background:#EDE9FE;border-radius:8px;font-size:10.5px;color:#5B21B6;line-height:1.7;">
        📌 <b>เกณฑ์การนับ Cancellation Rate:</b> นับเฉพาะงานที่ถูก<b>ยกเลิกโดยบุคคลในฝ่ายเขียนแบบ</b> และ<b>ยกเลิกหลังจากเริ่มเขียนงานจริงแล้ว</b> (ผ่านอนุมัติ Lv.2 แล้ว) เท่านั้น
        ${cancelExcludedCount + rejectedBackCount > 0 ? `<br>
        ⛔ <b>ไม่นับเข้า KPI:</b>
        ${preStartCancelCount > 0 ? `ยกเลิกก่อนเริ่มเขียนงาน <b>${preStartCancelCount}</b> รายการ · ` : ""}
        ${outsideCancelCount > 0 ? `ยกเลิกโดยคนนอกฝ่าย <b>${outsideCancelCount}</b> รายการ · ` : ""}
        ${rejectedBackCount > 0 ? `ตีกลับ/ส่งกลับผู้ร้องขอ <b>${rejectedBackCount}</b> รายการ` : ""}` : ""}
      </div>

      <!-- KPI Cards -->
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px;">
        ${kpiCard("🚫","Cancellation Rate",`${cancelRate}%`,`${cancelCount} งาน จาก ${total} คำร้อง (ไม่นับ ${cancelExcludedCount + rejectedBackCount} รายการที่ไม่กระทบ KPI)`,cancelRate<=5?"#0DB14B":cancelRate<=15?"#F59E0B":"#EF4444")}
