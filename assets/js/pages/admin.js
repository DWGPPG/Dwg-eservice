...
import { getDrawingTeamMembers, getAllManagerEmails , getKnownRequesters } from "../services/team-service.js";
...
import { openRequestDetail } from "./track.js";
...
function openTransferRequesterModal(view, state, request) {
  // รวมผู้ที่เคยส่งคำร้องจริง + สมาชิกฝ่ายเขียนแบบ (ผู้ร้องขอส่วนใหญ่ไม่ได้อยู่ใน DrawingTeam)
  const team = getKnownRequesters(request.requesterEmail);
  const body = `
  ...
  return `
    <tr>
      ...
      <td data-label="เลขที่คำร้อง">
        <button class="track-request-link" type="button" data-request-detail="${escapeHtml(item.requestNo)}"
          title="คลิกเพื่อดูใบคำขอและรายละเอียดทั้งหมด"
          style="white-space:nowrap;font-weight:700;">${escapeHtml(item.requestNo)}</button><br>
      ...
      <td data-label="ผู้ขอ">${escapeHtml(item.requesterName || "—")}</td>
