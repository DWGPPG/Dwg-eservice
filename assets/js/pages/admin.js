...
function renderFilterBar(state, allItems) {
  ...
  const hasActive = adminProjectFilter || adminAssigneeFilter || adminDateFrom || adminDateTo;
  return `
    <div class="admin-toolbar">
      ...
      <div class="filter-row" style="flex-wrap:wrap;gap:8px;margin-top:8px;align-items:center;">
        <input id="admin-search" class="search-input" type="search"
          placeholder="🔍 ค้นหาเลขคำร้อง โครงการ ชื่อ..."
          value="${escapeHtml(adminSearchQuery)}" style="min-width:220px;" />

        <select id="admin-project-filter" class="search-input" style="min-width:160px;">
          <option value="">📂 ทุกโครงการ</option>
          ${projects.map((p) => `<option value="${escapeHtml(p)}" ${adminProjectFilter === p ? "selected" : ""}>${escapeHtml(p)}</option>`).join("")}
        </select>

        <select id="admin-assignee-filter" class="search-input" style="min-width:160px;">
        ...
        <label style="display:flex;align-items:center;gap:4px;font-size:13px;color:#555;">
          ตั้งแต่ <input id="admin-date-from" type="date" value="${adminDateFrom}"
            style="border:1px solid #d0d7de;border-radius:6px;padding:4px 8px;font-size:13px;" />
        </label>
        <label style="display:flex;align-items:center;gap:4px;font-size:13px;color:#555;">
          ถึง <input id="admin-date-to" type="date" value="${adminDateTo}"
            style="border:1px solid #d0d7de;border-radius:6px;padding:4px 8px;font-size:13px;" />
        </label>

        ${hasActive ? `<button id="admin-clear-filters" class="secondary-button" type="button" style="height:34px;padding:0 12px;font-size:13px;">✕ ล้าง</button>` : ""}
      </div>
    </div>
  `;
}
  ...
  const body = `
    <div class="assign-work-panel">
      ...
      <div style="background:#fef3c7;border:1px solid #fcd34d;border-radius:8px;padding:12px 16px;margin:12px 0;font-size:14px;color:#92400e;">
        ...
        การดึงงานจะแจ้งเตือนเจ้าของงานเดิมและผู้จัดการทาง Teams โดยอัตโนมัติ
      </div>
      <label class="field">
        ...
        onClick: async (close) => {
          ...
          try {
            await patchRequest(request, {
              ...
              assignNote: `[ดึงงานจาก ${request.assignedToName || request.assignedToEmail}] ${fullReason}`,
            }, `ดึงงานจาก ${request.assignedToName || "—"} — ${fullReason}`);
            showToast(`ดึงงาน ${request.requestNo} มาแล้ว — แจ้ง Teams เรียบร้อย`, "success");
            close();
