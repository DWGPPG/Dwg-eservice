import { STATUS, STATUS_LABELS , CANCEL_SOURCES , getCancelSource } from "../../../config/schema.js";
...
import { openRequestDetail, openTransferRequesterModal } from "./track.js";
...
// ── เรียงรายการล่าสุดอยู่บนสุด (ใช้ร่วมทุก Tab: รับงาน / อนุมัติเริ่มงาน / ตรวจสอบส่งมอบ / งานในฝ่าย) ──
function sortNewestFirst(items) {
  return [...items].sort((a, b) => {
    const ta = new Date(a.submittedAt || 0).getTime() || 0;
    const tb = new Date(b.submittedAt || 0).getTime() || 0;
    if (tb !== ta) return tb - ta;
    // ถ้าเวลาส่งเท่ากัน (ส่งชุดเดียวกัน) ให้เรียงตามเลขที่คำร้องจากมากไปน้อย
    return String(b.requestNo || "").localeCompare(String(a.requestNo || ""), undefined, { numeric: true });
  });
}
...
function applyAllFilters(items) {
  return sortNewestFirst(
    applySearchFilter(applyProjectFilter(applyAssigneeFilter(applyDateFilter(applyTypeFilter(items)))))
  );
}
...
function renderDesignerPickup(view, state) {
  ...
  view.innerHTML = `
    <section class="content-section admin-page">
      ...
      <div class="admin-jump-row" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin:4px 0 20px;padding-bottom:18px;border-bottom:1px solid #e2e8f0;">
        ${jumpButton("pickup-section",   "🙋 รับงาน",               allPending.length, false, "pickup-bulk-button", "pickup-bulk-count")}
      ...
      <div id="pickup-section" ${sectionHidden("pickup-section")}>
        ...
        ${renderPickupTableSection(pendingItems, "รอฝ่ายแบบรับงาน", false, state)}
      </div>
  ...
  // ── tab switching (ใช้ data-jump-to เหมือน manager view) ──
  view.querySelectorAll("[data-jump-to]").forEach((btn) => {
    const targetId = btn.dataset.jumpTo;
    if (view._adminSection === targetId) btn.style.boxShadow = "0 0 0 2px rgba(0,93,172,0.5) inset";
    btn.addEventListener("click", () => {
      view._adminSection = view._adminSection === targetId ? "pickup-section" : targetId;
      renderDesignerPickup(view, state);
    });
  });
  ...
  bindRequestDetailEvents(view, state);
...
function renderManagerAdmin(view, state) {
  ...
  const mgrReviewItems = sortNewestFirst(state.requests.filter((i) => i.status === STATUS.MGR_REVIEW));
  ...
  view.innerHTML = `
    <section class="content-section admin-page">
      ...
      <div class="admin-section-block" id="pickup-section" ${sectionHidden("pickup-section")}>
        ...
        ${renderPickupTableSection(pendingItems, "รอฝ่ายแบบรับงาน", true, state)}
      </div>
...
function renderTakeoverSection(items, state) {
  ...
  // เรียง "รายการล่าสุดอยู่บนสุด" ตามวันที่ส่งคำร้อง
  const notMine = sortNewestFirst(
    items.filter((i) => (i.assignedToEmail || "").toLowerCase() !== myEmail)
  );
  return `
    <div class="pickup-section" id="takeover-inner">
      ...
      <div class="track-table-wrap">
        <table class="track-sheet pickup-table">
          ...
          <tbody>
            ${notMine.length
              ? notMine.map((item) => renderTakeoverRow(item, state)).join("")
...
function renderTakeoverRow(item, state) {
  const due  = pickupDueLabel(item.dueDate, getHolidaySet(state));
  ...
  return `
    <tr data-takeover-row data-project="${escapeHtml(item.projectName||"")}" data-assignee="${escapeHtml(item.assignedToName||"")}" data-type="${escapeHtml(item.requestType||"")}">
      <td data-label="เลขที่คำร้อง" style="white-space:nowrap;">
        <button class="track-request-link" type="button" data-request-detail="${escapeHtml(item.requestNo)}"
          title="คลิกเพื่อดูใบคำขอและรายละเอียดทั้งหมด"
          style="font-weight:700;">${escapeHtml(item.requestNo)}</button>
      </td>
      <td data-label="Drawing">
        <strong style="display:block;overflow-wrap:break-word;">${escapeHtml(item.drawingNo || "—")}</strong>
        <small style="display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;color:#64748B;line-height:1.4;margin-top:2px;max-width:210px;"
          title="${escapeHtml(item.drawingName || "")}">${escapeHtml(item.drawingName || "")}</small>
      </td>
      <td data-label="ผู้ร้องขอ">
      ...
      <td data-label="ผู้รับผิดชอบปัจจุบัน">
      ...
      <td data-label="โครงการ">${escapeHtml(item.projectName || "—")}</td>
      <td data-label="กำหนดส่ง"><span class="due-chip ${due.className}">${escapeHtml(due.label)}</span></td>
      ...
      <td data-label="ดำเนินการ" style="white-space:nowrap;">
...
// ── เปิด popup "ใบคำขอ + รายละเอียด" เมื่อคลิกเลขที่คำร้อง (ทุก Tab ในหน้ารับงาน/อนุมัติ) ──
function bindRequestDetailEvents(view, state) {
  view._requestDetailEvents?.abort();
  const controller = new AbortController();
  view._requestDetailEvents = controller;

  view.addEventListener("click", (event) => {
    const btn = event.target.closest("[data-request-detail]");
    if (!btn) return;
    event.preventDefault();
    event.stopPropagation();
    const request = state.requests.find((i) => i.requestNo === btn.dataset.requestDetail);
...
function bindTakeoverEvents(view, state) {
  view.addEventListener("click", async (event) => {
    ...
    if (transferBtn) {
      ...
      const request   = state.requests.find((i) => i.requestNo === requestNo);
      if (!request) return;
      openTransferRequesterModal(view, state, request, () => renderAdmin(view, state));
    }
  });
}
...
async function notifyTakeover({ request, me, prevOwnerName, prevOwnerEmail, fullReason, state }) {
  ...
  // ── ช่องทาง 4: กลุ่ม Nexus (เฉพาะงาน Proposal) ──
  const nexusMsg = [
    `🔄 <b>โอนงานภายในฝ่าย</b>`,
    ``,
  ...
  tasks.push(
    notifyNexusGroup(request, `🔄 Proposal — โอนงานภายในฝ่าย`, nexusMsg).catch((err) =>
      console.warn("notifyTakeover → Nexus failed:", err.message)
    )
  );
...
function renderPickupTableSection(items, heading, isManager = false, state = null) {
  return `
    <div class="pickup-section">
      ...
      <div class="track-table-wrap">
        <table class="track-sheet pickup-table">
          ...
          <tbody>
            ${items.length
              ? items.map((item) => pickupRow(item, isManager, state)).join("")
...
function pickupRow(item, isManager = false, state = null) {
  const due = pickupDueLabel(item.dueDate, getHolidaySet(state));
  ...
  return `
    <tr>
      <td class="pickup-select-cell"><input type="checkbox" class="pickup-checkbox" data-pickup-id="${escapeHtml(item.requestNo)}" /></td>
      <td data-label="กำหนด"><span class="due-chip ${due.className}">${escapeHtml(due.label)}</span></td>
      <td data-label="เลขที่คำร้อง" style="overflow-wrap:break-word;">
        <button class="track-request-link" type="button" data-request-detail="${escapeHtml(item.requestNo)}"
          title="คลิกเพื่อดูใบคำขอและรายละเอียดทั้งหมด"
          style="overflow-wrap:break-word;font-weight:700;white-space:normal;text-align:left;">${escapeHtml(item.requestNo)}</button><br>
        <small style="color:#005DAC;font-weight:600;overflow-wrap:break-word;">${escapeHtml(item.projectName || "—")}</small>
      </td>
      <td data-label="Drawing">
        <strong style="display:block;overflow-wrap:break-word;">${escapeHtml(item.drawingNo || "—")}</strong>
        <small style="display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;color:#64748B;line-height:1.4;margin-top:2px;max-width:210px;"
      ...
      <td data-label="ประเภทงาน"><span class="badge badge-pending">${escapeHtml(item.requestType || "—")}</span></td>
      <td data-label="ผู้ขอ" style="overflow-wrap:break-word;">${escapeHtml(item.requesterName || "—")}</td>
      <td data-label="วันที่/เวลาส่งคำขอ" style="white-space:nowrap;font-size:13px;color:#555;">${escapeHtml(submittedLabel)}</td>
      <td data-label="โฟลเดอร์ / ลิงก์" style="white-space:nowrap;">
        <a href="${escapeHtml(folder)}" target="_blank" rel="noopener noreferrer"
          class="mgr-file-btn folder-btn" style="display:inline-flex;align-items:center;gap:4px;font-size:12px;padding:3px 8px;">
          📁 โฟลเดอร์
      ...
      <td data-label="สาเหตุการแก้ไข" style="min-width:160px;max-width:220px;">
      ...
      <td data-label="รับงาน" style="white-space:nowrap;text-align:center;">
      ...
      <td data-label="อื่นๆ" style="white-space:nowrap;text-align:center;">
        ...
        <button class="small-button secondary-button" data-transfer-requester="${escapeHtml(item.requestNo)}" type="button" style="margin-top:4px;"
          title="โอนคำร้องนี้ให้ผู้ร้องขอคนใหม่ (กรณีผู้ร้องขอเดิมลาออก/ย้ายงาน)">👤 โอนผู้ร้องขอ</button>
      </td>
    </tr>
...
function pickupDueLabel(dueDate, holidaySet = new Set()) {
  ...
  // นับรวมวันนี้ + วันครบกำหนด (ยังทำได้ทั้งสองวัน)
  let workDays = 0;
  const cur = new Date(today);
  const step = due >= today ? 1 : -1;
  while (cur.toDateString() !== due.toDateString()) {
    const dow = cur.getDay();
    if (dow !== 0 && dow !== 6 && !holidaySet.has(cur.toDateString())) workDays += step;
    cur.setDate(cur.getDate() + step);
  }
  const endDow = due.getDay();
  if (endDow !== 0 && endDow !== 6 && !holidaySet.has(due.toDateString())) workDays += step;
  if (workDays < 0) return { label: `เกิน ${Math.abs(workDays)} วันทำงาน`, className: "overdue" };
  return { label: `เหลือ ${workDays} วันทำงาน`, className: "soon" };
}
    ...
    <article class="admin-card" id="admin-card-${escapeHtml(item.requestNo)}">
      <div class="admin-card-header">
        <div>
          ...
          <div class="admin-card-ref">
            <button class="track-request-link" type="button" data-request-detail="${escapeHtml(item.requestNo)}"
              title="คลิกเพื่อดูใบคำขอและรายละเอียดทั้งหมด"
              style="font-weight:700;">${escapeHtml(item.requestNo)}</button>
            ${item.drawingName ? "— " + escapeHtml(item.drawingName) : ""}
          </div>
        </div>
        ${statusBadge}
      ...
      <div id="admin-action-box-${escapeHtml(item.requestNo)}" class="admin-action-box" hidden>
        ...
        <div id="admin-cancel-src-wrap-${escapeHtml(item.requestNo)}" hidden>
          ...
          <select id="admin-cancel-src-${escapeHtml(item.requestNo)}" style="width:100%;padding:8px;border:1.5px solid #FCA5A5;background:#FEF2F2;border-radius:6px;font-size:13px;">
            <option value="">— เลือกต้นเหตุ —</option>
            ${CANCEL_SOURCES.map((s) => `<option value="${s.code}">${s.emoji} ${escapeHtml(s.label)}</option>`).join("")}
          </select>
          <div id="admin-cancel-src-${escapeHtml(item.requestNo)}-hint" style="margin-top:5px;font-size:10.5px;color:#B91C1C;line-height:1.5;">
...
function toggleActionBox(view, requestNo, mode, isManager, hasLv1) {
  ...
  const srcWrap = view.querySelector(`#admin-cancel-src-wrap-${cssEscape(requestNo)}`);
  const srcSel  = view.querySelector(`#admin-cancel-src-${cssEscape(requestNo)}`);
  if (srcWrap) srcWrap.hidden = mode !== "cancel";
  if (srcSel) {
    srcSel.value = "";
    const hint = view.querySelector(`#admin-cancel-src-${cssEscape(requestNo)}-hint`);
    srcSel.onchange = () => {
      const meta = getCancelSource(srcSel.value);
...
async function submitActionBox(view, state, request, isManager) {
  ...
  const cancelSrc  = view.querySelector(`#admin-cancel-src-${cssEscape(requestNo)}`)?.value || "";
  if (mode === "cancel" && !cancelSrc) { showToast("กรุณาเลือกต้นเหตุการยกเลิก", "warning"); return; }
  const confirmBtn = view.querySelector(`#admin-action-confirm-${cssEscape(requestNo)}`);
  confirmBtn.disabled = true; confirmBtn.classList.add("is-loading");
  try {
    if (mode === "cancel") {
      await adminCancel(request, reason, cancelSrc);
  ...
  return `
    <article class="admin-card" id="mgr-card-${escapeHtml(item.requestNo)}">
      <div class="admin-card-header">
        <div>
          <div class="admin-card-title">
            <button class="track-request-link" type="button" data-request-detail="${escapeHtml(item.requestNo)}"
              title="คลิกเพื่อดูใบคำขอและรายละเอียดทั้งหมด"
              style="font-size:inherit;font-weight:inherit;">${escapeHtml(item.requestNo)}</button>
          </div>
      ...
      <div id="mgr-reject-box-${escapeHtml(item.requestNo)}" class="admin-action-box tone-warning" hidden>
        <div class="admin-action-label">↩️ ส่งกลับแก้ไข — กรอกข้อมูลให้ครบทุกช่อง</div>

        <label style="display:block;font-size:12px;font-weight:600;color:#92400E;margin:8px 0 4px;">
          1️⃣ สาเหตุการแก้ไข <b style="color:#EF4444;">*</b>
        </label>
        <select id="mgr-reject-source-${escapeHtml(item.requestNo)}"
          style="width:100%;padding:8px;border:1.5px solid #FCD34D;background:#FFFBEB;border-radius:6px;font-size:13px;">
        ...
        <div id="mgr-reject-src-note-${escapeHtml(item.requestNo)}"
          style="display:none;margin-top:6px;padding:7px 10px;background:#FEE2E2;border:1px solid #FCA5A5;border-radius:6px;font-size:11px;color:#991B1B;">
          ⚠️ สาเหตุ 1.1 จะถูกนับเข้า KPI คุณภาพงานของฝ่ายเขียนแบบ
        </div>

        <label style="display:block;font-size:12px;font-weight:600;color:#92400E;margin:12px 0 4px;">
          2️⃣ เหตุผล / จุดที่ต้องแก้ไข <b style="color:#EF4444;">*</b>
        </label>
        <textarea id="mgr-reject-note-${escapeHtml(item.requestNo)}" rows="3" placeholder="ระบุจุดที่ต้องแก้ไขให้ชัดเจน..."></textarea>

        <label style="display:block;font-size:12px;font-weight:600;color:#92400E;margin:12px 0 4px;">
          3️⃣ แนบไฟล์ประกอบ (มาร์กอัป/รูป/PDF) <b style="color:#EF4444;">*</b>
        </label>
        <input type="file" id="mgr-reject-files-${escapeHtml(item.requestNo)}" multiple
          style="width:100%;padding:7px;border:1.5px dashed #FCD34D;background:#FFFBEB;border-radius:6px;font-size:12px;" />
        <div id="mgr-reject-filelist-${escapeHtml(item.requestNo)}" style="font-size:11px;color:#92400E;margin-top:4px;"></div>

        <label style="display:block;font-size:12px;font-weight:600;color:#92400E;margin:12px 0 4px;">
          4️⃣ กำหนดส่งใหม่ <b style="color:#EF4444;">*</b>
        </label>
        <input type="date" id="mgr-reject-due-${escapeHtml(item.requestNo)}"
          style="width:100%;padding:8px;border:1.5px solid #FCD34D;background:#FFFBEB;border-radius:6px;font-size:13px;" />
        <div id="mgr-reject-due-warn-${escapeHtml(item.requestNo)}" style="display:none;"></div>

        <div class="admin-action-buttons" style="margin-top:14px;">
...
function bindMgrReviewEvents(view, state) {
  ...
  view.addEventListener("click", async (e) => {
    ...
    if (action === "toggle-reject") {
      ...
      if (box && !box.hidden) {
        // แสดงคำเตือนเมื่อเลือกสาเหตุ 1.1
        const src = view.querySelector(`#mgr-reject-source-${safeId}`);
        const srcNote = view.querySelector(`#mgr-reject-src-note-${safeId}`);
        if (src && srcNote) src.onchange = () => { srcNote.style.display = src.value === "1.1" ? "block" : "none"; };

        // แสดงรายชื่อไฟล์ที่เลือก
        const fileEl = view.querySelector(`#mgr-reject-files-${safeId}`);
        const fileList = view.querySelector(`#mgr-reject-filelist-${safeId}`);
        if (fileEl && fileList) {
          fileEl.onchange = () => {
            const names = [...(fileEl.files || [])].map((f) => f.name);
            fileList.innerHTML = names.length
              ? `📎 เลือกแล้ว ${names.length} ไฟล์: ${escapeHtml(names.join(", "))}`
              : "";
          };
        }

        // เตือนทันทีเมื่อเลือกกำหนดส่งใหม่เป็นวันนี้
        const dueEl = view.querySelector(`#mgr-reject-due-${safeId}`);
        const dueWarn = view.querySelector(`#mgr-reject-due-warn-${safeId}`);
        if (dueEl && dueWarn) {
          const now = new Date();
          const todayKey = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}`;
          dueEl.min = todayKey;
          dueEl.onchange = () => {
            if (dueEl.value === todayKey) {
              ...
              dueWarn.innerHTML = `<div style="margin-top:7px;padding:9px 12px;background:#FEF2F2;border:1.5px solid #DC2626;border-radius:8px;font-size:11.5px;color:#B91C1C;font-weight:600;line-height:1.6;">
                ...
                <span style="font-weight:400;">ยังยืนยันใช้วันนี้ได้ ระบบจะถามยืนยันอีกครั้งตอนกดส่งกลับ</span>
              </div>`;
            } else {
    ...
    if (action === "confirm-reject") {
      const source = view.querySelector(`#mgr-reject-source-${safeId}`)?.value || "";
      const reason = view.querySelector(`#mgr-reject-note-${safeId}`)?.value.trim() || "";
      const fileEl = view.querySelector(`#mgr-reject-files-${safeId}`);
      const files  = fileEl?.files ? [...fileEl.files] : [];
      const newDue = view.querySelector(`#mgr-reject-due-${safeId}`)?.value || "";

      // ── บังคับกรอกให้ครบทุกช่อง ──
      if (!source)        { showToast("กรุณาเลือกสาเหตุการแก้ไข (1.1–1.4)", "warning"); return; }
      if (!reason)        { showToast("กรุณาระบุเหตุผล / จุดที่ต้องแก้ไข", "warning"); return; }
      ...
      // ── เตือนถ้ากำหนดส่งใหม่เป็นวันเดียวกันกับวันที่ส่งกลับ (เตือนเฉยๆ ยังไปต่อได้) ──
      const now = new Date();
      const todayKey = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-${String(now.getDate()).padStart(2,"0")}`;
      if (newDue === todayKey) {
      ...
      btn.disabled = true; btn.classList.add("is-loading");
      try {
        await mgrRejectWork(request, reason, { revisionSource: source, newDueDate: newDue, files });
        showToast(`ส่งกลับแก้ไข ${requestNo} แล้ว — กำหนดส่งใหม่ ${newDue}`, "warning");
        renderAdmin(view, state);
      } catch (err) {
        showToast(`เกิดข้อผิดพลาด: ${err.message}`, "error");
        btn.disabled = false; btn.classList.remove("is-loading");
