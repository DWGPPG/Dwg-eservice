...
function renderSelectedDrawingQueue(view) {
      ...
      <article class="selected-drawing-card" data-selected-drawing="${escapeHtml(item.key)}">
        ...
        <div class="selected-drawing-source-fields">
          ...
          <label class="drawing-file-drop ${detail.files?.length ? "has-files" : ""}" data-file-drop="${escapeHtml(item.key)}">
            <input data-detail="files" type="file" multiple accept=".dwg,.pdf,*/*" />
            <span class="drawing-file-drop-icon">⇧</span>
            <span class="drawing-file-drop-copy">
              <strong>${detail.files?.length ? `${detail.files.length} ไฟล์ — คลิกเพื่อเพิ่มไฟล์อีก` : "ลากไฟล์มาวาง หรือคลิกเลือก (แนบได้หลายไฟล์)"}</strong>
              <small>${detail.files?.length ? "" : "รองรับทุกประเภทไฟล์ — รูปภาพ, PDF, DWG, Office, ZIP และอื่นๆ — ขนาดสูงสุด 1 GB"}</small>
            </span>
          </label>
          ${detail.files?.length ? `
        ...
        <div class="revision-source-block" style="margin-top:10px;padding:12px 14px;background:#FFF7ED;border:1.5px solid #FCD34D;border-radius:10px;">
          ...
          <label class="field" style="margin-bottom:8px;">
            <span style="font-size:12px;color:#92400E;">สาเหตุการแก้ไข</span>
            <select data-detail="revisionSource" style="border-color:#FCD34D;background:#FFFBEB;">
  ...
  target.querySelectorAll("[data-file-drop]").forEach((dropZone) => {
    ...
    const addFiles = (files) => {
      if (!drawingDetails[key]) drawingDetails[key] = {};
      const existing = drawingDetails[key].files || [];
  ...
  target.querySelectorAll("[data-remove-file]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (drawingDetails[key]?.files) {
        drawingDetails[key].files.splice(idx, 1);
        renderSelectedDrawingQueue(view);
      }
    });
  });
...
async function submitConfirmed(view, form) {
  ...
  const allFiles = selectedDrawings.map((item) => getDetail(item.key).files || []);
  ...
  for (const item of selectedDrawings) {
    ...
    try {
      ...
      }, detail.files || [], ({ fileIndex, fileName, percent }) => {
