        ...
        <div class="selected-drawing-source-fields">
          ...
          <label class="drawing-file-drop ${detail.files?.length ? "has-files" : ""}" data-file-drop="${escapeHtml(item.key)}">
            ...
            <span class="drawing-file-drop-copy">
              ...
              <small>${detail.files?.length ? escapeHtml(detail.files.map((f) => `${f.name} (${formatFileSize(f.size)})`).join(", ")) : "รองรับทุกประเภทไฟล์ — รูปภาพ, PDF, DWG, Office, ZIP และอื่นๆ — ขนาดสูงสุด 1 GB"}</small>
            </span>
          </label>
        </div>
