...
function bindSubmitFlow(view) {
  ...
  view.querySelectorAll("input[name='requestType']").forEach((radio) => {
    radio.addEventListener("change", () => {
      ...
      // Step 1 → โชว์ชื่อโครงการเมื่อเลือกประเภทแล้ว
      // Drawing section และ queue ยังซ่อนอยู่ รอให้เลือกโครงการก่อน
      ...
      if (projectWrapper) {
        ...
        // ถ้าเพิ่งเปลี่ยนประเภท ให้ซ่อน drawing+queue ไปก่อนถ้ายังไม่ได้เลือกโครงการ
...
function clearProject(view) {
  ...
  // คืน project wrapper กลับเป็น hidden style (ยังคงอยู่ใน grid ไม่หาย)
  const projectWrapper = view.querySelector("#project-select-wrapper");
  if (projectWrapper) {
    projectWrapper.classList.add("submit-project-field-hidden");
    projectWrapper.classList.remove("submit-project-field-visible");
  }
