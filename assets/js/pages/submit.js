...
function renderSelectedDrawingQueue(view) {
  ...
  // ── ปุ่ม "+ เพิ่มไฟล์" — เปิด file dialog ซ้ำเพื่อเพิ่มไฟล์จากโฟลเดอร์อื่น ──
  target.querySelectorAll("[data-add-more-files]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      const key = btn.dataset.addMoreFiles;
      const dropZone = target.querySelector(`[data-file-drop="${key}"]`);
      const input = dropZone?.querySelector("input[type='file']");
