...
let composerDraft = "";   // เก็บข้อความที่พิมพ์ค้างไว้ กันหายเวลา paint() วาดใหม่ (เช่น ตอนกดเลือกอารมณ์)
let editingId = null;
...
// ── การรีเฟรชอัตโนมัติ ──
const REFRESH_INTERVAL_MS = 30000; // ดึงโพสต์ใหม่ทุก 30 วินาที
...
export async function renderTeamPulse(container, state) {
  ...
  // มีข้อมูลเดิมอยู่แล้ว → วาดทันทีไม่ต้องโชว์ skeleton ให้กระพริบ แล้วค่อยดึงของใหม่เบื้องหลัง
  if (cachedPosts === null) {
    ...
    paint(container, state);
    refreshPosts(); // ดึงของใหม่ทุกครั้งที่กลับเข้าหน้านี้ ไม่ต้องรอ polling รอบถัดไป
  }
...
/**
 * ดึงโพสต์ใหม่จาก SharePoint แล้ววาดใหม่เฉพาะเมื่อข้อมูลเปลี่ยนจริง
 * ไม่วาดทับขณะผู้ใช้กำลังทำงานอยู่ เพื่อไม่ให้สิ่งที่พิมพ์ค้างไว้หายไปกลางคัน
 */
async function refreshPosts() {
  if (isRefreshing || !lastContainer || !lastState) return;

  // กันวาดทับระหว่างที่ผู้ใช้กำลังแก้ไข/ลบ/เลือก reaction อยู่
  if (editingId !== null || confirmingDeleteId !== null || reactionPickerOpenFor !== null) return;

  // กันวาดทับตอนกำลังพิมพ์อยู่ในช่องโพสต์
  const composer = lastContainer.querySelector("#tp-composer-ta");
  if (composerDraft.trim() || (composer && document.activeElement === composer)) return;

  // ป็อปอัพเช็คอินเปิดอยู่ → ยังไม่ต้องวาดทับ
  if (document.querySelector(".modal-backdrop")) return;

  isRefreshing = true;
  try {
    const fresh = await loadTeamPulsePosts();
    // เทียบก่อนว่าเปลี่ยนจริงไหม จะได้ไม่วาดใหม่โดยไม่จำเป็น (กันหน้ากระพริบ)
    if (JSON.stringify(fresh) !== JSON.stringify(cachedPosts)) {
      cachedPosts = fresh;
      paint(lastContainer, lastState);
    }
  } catch (err) {
    console.warn("Team Pulse: รีเฟรชฟีดไม่สำเร็จ (ไม่กระทบการใช้งาน):", err.message);
  } finally {
    isRefreshing = false;
  }
}
...
function startAutoRefresh() {
  ...
  refreshTimer = setInterval(() => {
    // แท็บถูกซ่อนอยู่ → ไม่ต้องยิงขอข้อมูล ประหยัดทั้งเครือข่ายและแบตเตอรี่
    if (document.visibilityState !== "visible") return;
    // widget ถูกถอดออกจากหน้าแล้ว (ผู้ใช้ไปหน้าอื่น) → หยุดรีเฟรช
    if (!lastContainer || !lastContainer.isConnected) { stopAutoRefresh(); return; }
    refreshPosts();
  }, REFRESH_INTERVAL_MS);

  // กลับมาที่แท็บ/สลับแอปกลับมา → ดึงของใหม่ทันที ไม่ต้องรอครบรอบ
  visibilityHandler = () => {
...
function paint(container, state) {
  ...
  const iAmManager = canModerateTeamPulse(user);   // เคารพมุมมองที่กำลังดูด้วย
    ...
    <div class="tp-card-shell">
      ...
      ${canPost ? `
        <div class="tp-composer">
          ...
          <textarea id="tp-composer-ta" placeholder="วันนี้คุณรู้สึกอย่างไร แล้วจะทำอะไรบ้าง... (พิมพ์ #เลขคำร้อง เพื่อลิงก์ไปหน้ารายละเอียดได้)">${escapeHtml(composerDraft)}</textarea>
...
function bindComposer(container, state) {
  // จำข้อความที่พิมพ์ไว้ตลอด เพื่อให้ paint() วาดกลับมาได้เหมือนเดิม
  const ta = container.querySelector("#tp-composer-ta");
  if (ta) {
    ta.addEventListener("input", () => { composerDraft = ta.value; });
    // ถ้ามีร่างค้างอยู่ ให้วางเคอร์เซอร์ท้ายข้อความหลังวาดใหม่ (เช่น หลังกดเลือกอารมณ์)
    if (composerDraft) {
      const pos = composerDraft.length;
  ...
  container.querySelectorAll('[data-tp-mood-group="composer"] [data-tp-mood]').forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      paint(container, state);
      // คืนโฟกัสให้ช่องพิมพ์ ผู้ใช้จะได้พิมพ์ต่อได้เลยไม่ต้องคลิกซ้ำ
      container.querySelector("#tp-composer-ta")?.focus();
    });
  });

  container.querySelector("#tp-submit-btn")?.addEventListener("click", async () => {
    const input = container.querySelector("#tp-composer-ta");
    const text = (input?.value || "").trim();
    if (!text) { showToast("กรุณาพิมพ์ข้อความก่อนโพสต์", "warning"); input?.focus(); return; }
    ...
    try {
      ...
      selectedMood = null;
      composerDraft = "";   // โพสต์สำเร็จแล้วเคลียร์ร่างทิ้ง
