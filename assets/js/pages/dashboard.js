...
import { renderTeamPulse } from "./team-pulse-widget.js";
...
export function renderDashboard(view, state) {
  ...
  // ── Team Pulse Widget (เช็คอินความรู้สึก/แผนงานประจำวัน) — บนสุดของหน้า ──
  const teamPulseContainer = document.createElement("div");
  view.appendChild(teamPulseContainer);
  renderTeamPulse(teamPulseContainer, state).catch((err) => {
    console.warn("Team Pulse widget เกิดข้อผิดพลาดระหว่างทำงาน (ไม่กระทบส่วนอื่นของหน้า):", err.message);
    // หมายเหตุ: ไม่เคลียร์ innerHTML ทิ้งที่นี่ — ถ้า error เกิดขึ้น "หลัง" เนื้อหาถูก render สำเร็จไปแล้ว
    // (เช่น ขั้นตอนวัดความสูงฟีดหลัง paint) การเคลียร์ทิ้งจะทำให้เนื้อหาที่ถูกต้องหายไปโดยไม่จำเป็น
    // ปล่อยให้เนื้อหาที่มีอยู่ ณ ตอนนั้นแสดงต่อไป — กรณีเดียวที่จะยังค้างเป็น skeleton คือ error เกิดก่อน paint() ครั้งแรก
  });
  ...
  const refreshKpi = () => {
    ...
    if (body) body.innerHTML = renderKpiDashboard(state.requests || [], team, periodEl.value, person, customRange, state.masterData?.holidays || []);
    // เพิ่ม card รอรับงาน + ภาระสูงสุด + งานครบกำหนดวันนี้ + trend เดือนนี้ vs เดือนที่แล้ว
    const extraRow = body?.querySelector("#dash-extra-row");
    if (extraRow) {
      const now = new Date();
      ...
      const dueToday = allReqs.filter((r) => {
        if (!r.dueDate || [STATUS.DONE, STATUS.CANCELLED, STATUS.REJECTED].includes(r.status)) return false;
        return new Date(r.dueDate).toDateString() === todayStr;
      }).length;

      const thisMonthCount = allReqs.filter((r) => {
        const d = new Date(r.submittedAt || r.dueDate);
        return d.getFullYear() === thisYear && d.getMonth() === thisMonth;
      }).length;

      const lastMonthCount = allReqs.filter((r) => {
        const d = new Date(r.submittedAt || r.dueDate);
        return d.getFullYear() === lastMonthYear && d.getMonth() === lastMonth;
      }).length;

      const trendDiff = thisMonthCount - lastMonthCount;
      const trendPct  = lastMonthCount > 0 ? Math.round(Math.abs(trendDiff) / lastMonthCount * 100) : 0;
      const trendUp   = trendDiff >= 0;

      extraRow.innerHTML = `
        <div style="background:#fff;border-radius:12px;padding:16px 18px;border:1px solid #E2E8F0;box-shadow:0 2px 6px rgba(0,0,0,0.06);display:flex;flex-direction:column;gap:5px;">
          <div style="font-size:18px;">📥</div>
          <div style="font-size:11px;color:#64748B;font-weight:500;">รอฝ่ายเขียนแบบรับงาน</div>
          <div style="font-size:24px;font-weight:800;color:${queued>0?"#F59E0B":"#0DB14B"};line-height:1.1;">${queued}</div>
          <div style="font-size:10px;color:#94A3B8;">รอผู้รับผิดชอบ</div>
        </div>
        <div style="background:#fff;border-radius:12px;padding:16px 18px;border:1px solid #E2E8F0;box-shadow:0 2px 6px rgba(0,0,0,0.06);display:flex;flex-direction:column;gap:5px;">
          <div style="font-size:18px;">🏋️</div>
          <div style="font-size:11px;color:#64748B;font-weight:500;">ภาระสูงสุด</div>
          <div style="font-size:16px;font-weight:800;color:#7C3AED;line-height:1.2;">${escapeHtml((topWorkload.count>0?topWorkload.name:"—").split(" ")[0])}</div>
          <div style="font-size:10px;color:#94A3B8;">${topWorkload.count>0?`${topWorkload.count} งานค้าง · ${topWorkload.urgent} ด่วน`:"ไม่มีงานค้าง"}</div>
        </div>
        <div style="background:#fff;border-radius:12px;padding:16px 18px;border:1px solid ${dueToday>0?"#FEE2E2":"#E2E8F0"};box-shadow:0 2px 6px rgba(0,0,0,0.06);display:flex;flex-direction:column;gap:5px;${dueToday>0?"background:#FFF9F9;":""}">
          <div style="font-size:18px;">📅</div>
          <div style="font-size:11px;color:#64748B;font-weight:500;">ครบกำหนดวันนี้</div>
          <div style="font-size:24px;font-weight:800;color:${dueToday>0?"#EF4444":"#0DB14B"};line-height:1.1;">${dueToday}</div>
          <div style="font-size:10px;color:#94A3B8;">${dueToday>0?"งานที่ต้องส่งวันนี้":"ไม่มีงานครบกำหนด"}</div>
        </div>
        <div style="background:#fff;border-radius:12px;padding:16px 18px;border:1px solid #E2E8F0;box-shadow:0 2px 6px rgba(0,0,0,0.06);display:flex;flex-direction:column;gap:5px;">
          <div style="font-size:18px;">📊</div>
          <div style="font-size:11px;color:#64748B;font-weight:500;">เดือนนี้ vs เดือนที่แล้ว</div>
          <div style="display:flex;align-items:baseline;gap:6px;">
            <span style="font-size:24px;font-weight:800;color:#005DAC;line-height:1.1;">${thisMonthCount}</span>
            <span style="font-size:11px;color:${trendUp?"#0DB14B":"#EF4444"};font-weight:700;">${trendUp?"▲":"▼"} ${trendPct}%</span>
          </div>
          <div style="font-size:10px;color:#94A3B8;">เดือนที่แล้ว ${lastMonthCount} คำร้อง</div>
        </div>
      `;
    }
  };
  ...
  // ── Existing dashboard content (เหลือแค่กราฟ + คำร้องล่าสุด) ──
  const dashContainer = document.createElement("div");
  dashContainer.innerHTML = `
    <div class="dashboard-modern">
      <section class="dash-card yearly-pipeline-card" style="grid-column:1/-1;">
      ...
      <section class="dash-card team-list-card">
        <div class="section-header compact-header">
          <div>
            ...
            <p>งานคงค้างที่ยังต้องลงมือทำ (ไม่รวมงานที่ส่งเสร็จแล้ว)</p>
          </div>
        </div>
      ...
      <section class="dash-card flow-card">
        <div class="section-header compact-header">
          <div>
            ...
            <p>งานที่ยังต้องลงมือทำจริง — งานที่กดเสร็จสิ้นแล้วไม่นับเป็นภาระ</p>
          </div>
        </div>
...
// สถานะที่ถือว่า "ฝ่ายเขียนแบบทำงานเสร็จแล้ว" → ไม่นับเป็นภาระงานอีกต่อไป
// (mgr_review = ส่งให้ผู้จัดการตรวจแล้ว, delivered = รอผู้ส่งคำร้องตรวจรับ, done = ปิดงาน)
// หากถูกส่งกลับแก้ไข (mgr_rejected / working) จะกลับมานับเป็นภาระงานอีกครั้งโดยอัตโนมัติ
const NO_WORKLOAD_STATUSES = [
  STATUS.MGR_REVIEW, STATUS.DELIVERED, STATUS.DONE, STATUS.CANCELLED, STATUS.REJECTED,
];
...
function buildTeamWorkload(requests) {
  ...
  const workload = members.map((member) => {
    const memberRequests = requests.filter((item) =>
      ...
      && !NO_WORKLOAD_STATUSES.includes(item.status)
    );
