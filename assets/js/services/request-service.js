...
import { ensureRequestFolder, uploadRequestFiles } from "./drawing-service.js";
...
export async function patchRequest(request, patch, auditAction = "") {
  ...
  Object.entries(patch).forEach(([localKey, value]) => {
    ...
    if (spField && value !== undefined) spPatch[spField] = value;
  });
...
export async function adminCancel(request, reason) {
  ...
  const lines = [
    "❌ <b>PPG Drawing e-Service — ยกเลิกคำร้อง</b>", "",
    `📋 <b>เลขคำขอ:</b> ${request.requestNo}`,
    `📁 <b>โครงการ:</b> ${request.projectName || "—"}`,
    `📌 <b>เหตุผล:</b> ${reason}`,
...
export async function submitSendwork(request, { dwgUrl, pdfUrl, reviseTag, note, files = [] }) {
  ...
  if (files.length) {
    try {
      const uploaded = await uploadRequestFiles(request.requestNo, files, reviseTag);
...
async function notifyManagersForReview(request, { dwgUrl, pdfUrl, note }) {
  ...
  for (const mgrEmail of appConfig.approverLv2Emails) {
