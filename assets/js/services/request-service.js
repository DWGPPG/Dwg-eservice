...
export async function createRequest(formData, files = [], onProgress = null) {
  ...
  // แจ้งเตือน — ครอบ try/catch เพราะ notification error ไม่ควรทำให้การส่งคำขอพัง
  // คำร้องถูกบันทึกใน SharePoint แล้ว ณ จุดนี้
  try {
    await notifyNewRequest(request);
  } catch (notifyErr) {
    console.warn("notifyNewRequest failed (non-critical):", notifyErr.message);
  }
