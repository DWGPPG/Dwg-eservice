...
export async function notifyNexusGroup(request, title, htmlMessage) {
  ...
  const url = appConfig.teams?.nexusWebhookUrl;
  if (!url) {
    console.warn("notifyNexusGroup: ยังไม่ได้ตั้งค่า teams.nexusWebhookUrl ใน config.js — งาน Proposal จะยังไม่แจ้งเข้ากลุ่ม Nexus");
    return false;
  }
  try {
    return await sendGroupWebhook(url, title, htmlMessage);
  } catch (error) {
...
async function notifyNewRequest(request) {
  ...
  // ── 1) ลองส่งผ่าน Power Automate webhook ก่อน (รันด้วย identity ของเจ้าของ Flow
  //    ไม่ใช่ของ Requester เลยไม่ติดปัญหา "ผู้ส่งคำร้องไม่ได้เป็นสมาชิก Group Chat") ──
  const webhookUrl = appConfig.teams?.drawingTeamWebhookUrl;
  if (webhookUrl) {
    const webhookOk = await sendNewRequestWebhook(webhookUrl, {
    ...
    if (webhookOk) {
      console.log("notifyNewRequest: ส่งผ่าน webhook สำเร็จ");
      return;
    }
    console.warn("notifyNewRequest: webhook ส่งไม่สำเร็จ ลอง direct Graph ต่อ");
  }
