...
import { sendMail, sendTeams1on1, sendTeamsWebhook, sendNewRequestWebhook } from "../graph.js";
...
export async function requesterReview(request, action, comment = "", revisionSource = "") {
  ...
  if (action === "approve") {
    ...
    if (revisionSource) patch.revisionSource = revisionSource; // บันทึกสาเหตุ
  } else {
    patch.status = STATUS.WORKING;
  ...
  if (action === "reject") {
    ...
    // แจ้ง Group Chat "Drawing Dept. นะจ๊ะ"
    const GROUP_CHAT_ID = "19:e668fd9c854447918fdb8218e2f023bb@thread.v2";
    try {
      const token = await acquireToken();
