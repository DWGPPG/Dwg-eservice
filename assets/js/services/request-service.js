...
import { sendMail, sendTeams1on1, sendTeamsWebhook } from "../graph.js";
...
async function notifyNewRequest(request) {
  ...
  const title = request.isRevision
    ? `✏️ Revision เข้ามาแล้ว! — ${request.requestNo}`
    : `📋 คำร้องใหม่เข้ามาแล้ว! — ${request.requestNo}`;

  const facts = [
    ...(request.isRevision ? [{ name: "🔗 อ้างอิงจาก", value: request.refRequestId || "—" }] : []),
    { name: "📦 ประเภท",   value: request.requestType || "—" },
    { name: "🏗️ โครงการ", value: request.projectName || "—" },
    { name: "📐 Drawing",  value: `${request.drawingNo || "—"} — ${request.drawingName || "—"}` },
    { name: "👤 ผู้ส่ง",   value: request.requesterName || "—" },
    { name: "📅 วันที่ส่ง", value: thaiDate },
  ];

  const webhookUrl = appConfig.teams?.drawingTeamWebhookUrl;

  if (webhookUrl) {
    // ✅ วิธีหลัก — Incoming Webhook โพสต์เข้ากลุ่มได้โดยไม่ต้องเป็นสมาชิก
    await sendTeamsWebhook(webhookUrl, title, facts, appConfig.azure.redirectUri);
  } else {
    // ⚠️ Fallback — ถ้ายังไม่ได้ตั้ง webhook ให้ส่ง 1:1 หาผู้จัดการแทนชั่วคราว
    console.warn("drawingTeamWebhookUrl ยังไม่ได้ตั้งค่าใน config.js — fallback ส่ง 1:1 แทน");
    const html = [title, "", ...facts.map((f) => `<b>${f.name}:</b> ${f.value}`), "", `<a href="${appConfig.azure.redirectUri}">🖥️ เข้าระบบตรวจสอบ</a>`].join("<br>");
    const mgrEmails = getAllManagerEmails(appConfig.approverLv2Emails || []);
    await Promise.allSettled(mgrEmails.map((email) => sendTeams1on1(email, html)));
  }
}
