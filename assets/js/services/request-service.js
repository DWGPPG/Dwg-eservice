...
/**
 * โพสต์ข้อความเข้า Group Chat แบบทนทาน — ลองหลายชั้นจนกว่าจะสำเร็จ:
 *   1) ยิงตรงผ่าน Graph ด้วย token ของผู้ใช้ที่ล็อกอิน (ได้ผลถ้าผู้ใช้เป็นสมาชิกกลุ่ม — กรณี Nexus ใช่แน่)
 *   2) ถ้าไม่ผ่าน → relay ผ่าน Power Automate Flow เดิม (รันด้วย identity เจ้าของ Flow)
 * คืน true ถ้าช่องทางใดช่องทางหนึ่งสำเร็จ
 */
async function postToGroupChat(chatId, title, htmlMessage) {
  if (!chatId) return false;

  // ── ชั้นที่ 1: Graph direct (token ผู้ใช้) ──
  try {
    ...
    if (token && String(token).trim().length > 0) {
      ...
      if (resp.ok) {
        console.log("postToGroupChat: ยิงตรง Graph สำเร็จ →", chatId);
        return true;
      }
      const detail = await resp.text();
      console.warn("postToGroupChat: Graph direct ไม่ผ่าน", resp.status, detail, "→ ลอง relay flow");
    }
  } catch (e) {
    console.warn("postToGroupChat: Graph direct error:", e.message, "→ ลอง relay flow");
  }

  // ── ชั้นที่ 2: relay ผ่าน Flow เดิม ──
  const url = appConfig.teams?.drawingTeamWebhookUrl;
  if (url) {
    const ok = await sendGroupWebhook(url, {
      groupChatId: chatId,
      title: title || "",
      message: htmlMessage || "",
    });
    if (ok) {
      console.log("postToGroupChat: ยิงผ่าน relay flow สำเร็จ →", chatId);
      return true;
    }
    console.warn("postToGroupChat: relay flow ไม่ผ่าน →", chatId);
  }
...
export async function notifyNexusGroup(request, title, htmlMessage) {
  ...
  if (!chatId) {
    console.warn("notifyNexusGroup: ยังไม่ได้ตั้งค่า teams.nexusChatId ใน config.js");
    return false;
  }
  try {
    return await postToGroupChat(chatId, title, htmlMessage);
  } catch (error) {
...
function fromSharePoint(item) {
  ...
  return {
    ...
    revisionEvidence: item[f.revisionEvidence],
    ...
    cancelledByName: item[f.cancelledByName],
    cancelledByEmail: item[f.cancelledByEmail],
    cancelledStage: item[f.cancelledStage],
    cancelledSource: item[f.cancelledSource],
    cancelledAt: item[f.cancelledAt],
    deptFinishedAt: item[f.deptFinishedAt],
...
async function notifyNewRequest(request) {
  ...
  // ── งานประเภทอื่น → กลุ่ม Drawing Dept. (Graph direct → relay flow → 1:1) ──
  const drawingChatId = appConfig.teams?.drawingTeamChatId;
  const ok = await postToGroupChat(drawingChatId, title, msg);
  if (ok) {
    console.log("notifyNewRequest: ส่งเข้ากลุ่ม Drawing Dept. สำเร็จ");
    return;
  }
  console.warn("notifyNewRequest: เข้ากลุ่ม Drawing Dept. ไม่สำเร็จ — fallback 1:1 หาทีมแทน");
  await notifyTeam1on1(msg);
}
...
export async function patchRequest(request, patch, auditAction = "") {
  ...
  Object.entries(patch).forEach(([localKey, value]) => {
    ...
    // ถ้าค่าเป็น "" (ตั้งใจเคลียร์ฟิลด์วันที่) ต้องส่ง null ให้ Graph API ไม่ใช่ "" ตรงๆ
    // เพราะ SharePoint ปฏิเสธ empty string สำหรับ Date/Time field เสมอ (400 Invalid Request)
    if (DATE_LOCAL_KEYS.has(localKey)) {
      if (!value) {
        spPatch[spField] = null;
        return;
      }
      try {
  ...
  const saveResult = await patchItem(lists.requests, request.id, spPatch);
  updateRequest(request.requestNo, () => patch);

  // ถ้า SharePoint ปฏิเสธบาง field (เช่น ข้อความยาวเกินขีดจำกัดคอลัมน์) — patchItem จะข้ามให้เพื่อไม่ให้ทั้งคำร้องบันทึกไม่ผ่าน
  // แต่ต้องแจ้งผู้ใช้ด้วยว่าข้อมูลบางส่วนไม่ถูกบันทึกจริง ไม่ใช่ปล่อยให้เข้าใจผิดว่าสำเร็จครบ
  if (saveResult?.__skippedFields?.length) {
    const skippedLocalKeys = Object.entries(f)
      .filter(([, spName]) => saveResult.__skippedFields.includes(spName))
      .map(([localKey]) => localKey);
    console.warn(`patchRequest [${request.requestNo}]: บันทึกสำเร็จบางส่วน — ข้าม field: ${saveResult.__skippedFields.join(", ")} (local: ${skippedLocalKeys.join(", ")})`);
    showToast(`⚠️ บันทึกสำเร็จ แต่บางส่วนถูกข้าม (คอลัมน์อาจรับข้อความยาวเกินไป) — รบกวนแจ้งผู้ดูแลระบบ`, "warning");
  }
  if (auditAction) {
...
export async function adminCancel(request, reason, cancelledSource = "") {
  // เก็บข้อมูลไว้คำนวณ Cancellation KPI:
  //  • cancelledStage  = สถานะ ณ ตอนยกเลิก → ยกเลิกก่อนเริ่มเขียนงานจริงไม่นับเข้า KPI
  //  • cancelledSource = ต้นเหตุการยกเลิก → นับเข้า KPI เฉพาะ "requester" (ผู้ส่งคำร้อง)
  //                      เพราะฝ่ายเขียนแบบลงมือทำไปแล้วแต่ถูกสั่งยกเลิก = Wasted Cost
  const stageAtCancel = request.status || "";
  const srcLabel = cancelledSource === "requester" ? "ผู้ส่งคำร้อง"
                 : cancelledSource === "drawing"   ? "ฝ่ายเขียนแบบ" : "ไม่ระบุ";
  await patchRequest(request, {
    status: STATUS.CANCELLED,
    rejectReason: reason,
    cancelledStage: stageAtCancel,
    cancelledSource: cancelledSource || "",
    cancelledByName: state.user?.name || "",
    cancelledByEmail: state.user?.email || "",
    cancelledAt: new Date().toISOString(),
  }, `ยกเลิกคำร้อง (ต้นเหตุ: ${srcLabel})`);
...
export async function submitSendwork(request, { dwgUrl, pdfUrl, reviseTag, note, files = [], onProgress = null }) {
  ...
  await patchRequest(request, {
    ...
    // เวลาที่ฝ่ายเขียนแบบทำงานเสร็จจริง — ใช้วัด KPI ว่าส่งช้ากว่ากำหนดหรือไม่
    deptFinishedAt: new Date().toISOString(),
...
export async function mgrRejectWork(request, reason, options = {}) {
  const { revisionSource = "", newDueDate = "", files = [] } = options;
  const now = new Date().toISOString();

  // ── อัปโหลดไฟล์ประกอบการส่งกลับแก้ไข ──
  let evidenceLinks = [];
  if (Array.isArray(files) && files.length) {
    try {
      const uploaded = await uploadRequestFiles(request.requestNo, files);
      uploaded.forEach((file) => {
        const url = file.webUrl || file["@microsoft.graph.downloadUrl"] || "";
        if (url) evidenceLinks.push({ name: file.name, url });
      });
    } catch (err) {
      console.warn("mgrRejectWork: อัปโหลดไฟล์ไม่สำเร็จ:", err.message);
    }
  }
  const evidenceHtml = evidenceLinks.length
    ? evidenceLinks.map((f) => `📎 <a href="${f.url}">${f.name}</a>`).join("<br>")
    : "";
  ...
  const patch = {
    ...
    revisionCount: (parseInt(request.revisionCount || "0", 10) + 1).toString(),
  };
  if (revisionSource) patch.revisionSource = revisionSource;
  if (newDueDate) patch.dueDate = new Date(`${newDueDate}T00:00:00`).toISOString();

  await patchRequest(request, patch, `ผู้จัดการส่งกลับแก้ไข${revisionSource ? ` (สาเหตุ ${revisionSource})` : ""}${newDueDate ? ` · กำหนดส่งใหม่ ${newDueDate}` : ""}`);

  if (evidenceLinks.length) {
    try {
      await patchRequest(request, { revisionEvidence: JSON.stringify(evidenceLinks) });
    } catch (err) {
      console.warn("mgrRejectWork: บันทึก RevisionEvidence ไม่สำเร็จ:", err.message);
    }
  }

  const srcLabel = { "1.1":"1.1 ความผิดพลาดจากฝ่ายเขียนแบบ","1.2":"1.2 ความผิดพลาดจากผู้ส่งคำร้อง","1.3":"1.3 ความผิดพลาดจากงานติดตั้งโครงการ","1.4":"1.4 ความผิดพลาดจากลูกค้า" }[revisionSource] || revisionSource;
  const newDueTh = newDueDate
    ? new Date(`${newDueDate}T00:00:00`).toLocaleDateString("th-TH", { year: "numeric", month: "long", day: "numeric" })
    : "";
  ...
  const lines = [
    ...
    srcLabel ? `🏷️ <b>สาเหตุ:</b> ${srcLabel}` : "",
    newDueTh ? `📅 <b>กำหนดส่งใหม่:</b> ${newDueTh}` : "",
    evidenceHtml ? `<b>ไฟล์ประกอบ:</b><br>${evidenceHtml}` : "",
    "",
...
export async function requesterReview(request, action, comment = "", revisionSource = "", evidence = null) {
  const now = new Date().toISOString();
  const patch = { reviewedAt: now };

  // ── อัปโหลดหลักฐานแนบ (ไฟล์จริง + ลิงก์) เก็บเป็น JSON ใน RevisionEvidence ──
  let evidenceLinks = [];
  if (evidence && (action === "revise" || action === "reject")) {
    if (Array.isArray(evidence.files) && evidence.files.length) {
      try {
        const uploaded = await uploadRequestFiles(request.requestNo, evidence.files, evidence.onProgress || null);
        uploaded.forEach((file) => {
          const url = file.webUrl || file["@microsoft.graph.downloadUrl"] || "";
          if (url) evidenceLinks.push({ name: file.name, url });
        });
      } catch (err) {
        console.warn("requesterReview: อัปโหลดหลักฐานไม่สำเร็จ:", err.message);
      }
    }
    if (evidence.link && String(evidence.link).trim()) {
      evidenceLinks.push({ name: "🔗 ลิงก์แนบ", url: String(evidence.link).trim() });
    }
  }
  const evidenceHtml = evidenceLinks.length
    ? evidenceLinks.map((f) => `📎 <a href="${f.url}">${f.name}</a>`).join("<br>")
  ...
  if (action === "approve") {
    ...
    if (revisionSource) patch.revisionSource = revisionSource;
    // กำหนดส่งใหม่จากผู้ร้องขอ — งานกลับมาเป็น "กำลังดำเนินการ" พร้อมเดดไลน์ใหม่
    if (evidence?.newDueDate) {
      patch.dueDate = new Date(`${evidence.newDueDate}T00:00:00`).toISOString();
      patch.deptFinishedAt = "";  // เคลียร์เวลาส่งเสร็จเดิม เพราะต้องทำใหม่
    }
  } else {
    patch.status = STATUS.WORKING;
  ...
  // บันทึกหลักฐานแนบแยกต่างหาก (best-effort) — ถ้าฟิลด์ยาวเกิน/พลาด จะไม่กระทบการส่งกลับหลัก
  if (evidenceLinks.length) {
    try {
      await patchRequest(request, { revisionEvidence: JSON.stringify(evidenceLinks) });
    } catch (err) {
      console.warn("requesterReview: บันทึก RevisionEvidence ไม่สำเร็จ (ไม่กระทบการส่งกลับ):", err.message);
    }
  }
  ...
  const lines = [
    ...
    (action === "revise" && evidence?.newDueDate)
      ? `📅 <b>กำหนดส่งใหม่:</b> ${new Date(`${evidence.newDueDate}T00:00:00`).toLocaleDateString("th-TH", { year: "numeric", month: "long", day: "numeric" })}`
      : "",
    evidenceHtml ? `<b>หลักฐานแนบ:</b><br>${evidenceHtml}` : "",
    "",
