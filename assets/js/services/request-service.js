...
function toSharePoint(request) {
  ...
  // validate URL ก่อนส่งเป็น Hyperlink — ถ้าผิดรูปแบบให้ตัดทิ้งไม่ให้ error
  const hyperlink = (url) => {
    if (!url) return undefined;
    const trimmed = String(url).trim();
    if (!trimmed) return undefined;
    // SharePoint Hyperlink field รับเฉพาะ http/https และความยาวไม่เกิน 255 ตัวอักษร
    try {
      const parsed = new URL(trimmed);
      if (!["http:", "https:"].includes(parsed.protocol)) return undefined;
      const safeUrl = trimmed.length > 255 ? trimmed.slice(0, 255) : trimmed;
      return { Url: safeUrl, Description: safeUrl };
    } catch {
      return undefined; // URL ผิดรูปแบบ — ไม่ส่งไป SharePoint เพื่อหลีกเลี่ยง 400 error
    }
  };
...
async function notifyNewRequest(request) {
  ...
  // field ตรงตาม JSON Schema ที่ตั้งไว้ใน Power Automate Flow
  const payload = {
    title,
  ...
  if (webhookUrl) {
    try {
      ...
      if (!resp.ok) {
        ...
        console.warn("Power Automate notify failed:", resp.status, detail);
      }
    } catch (err) {
      console.warn("Power Automate notify error (non-critical):", err.message);
    }
  } else {
    console.warn("drawingTeamWebhookUrl ยังไม่ได้ตั้งค่าใน config.js");
  }
}
