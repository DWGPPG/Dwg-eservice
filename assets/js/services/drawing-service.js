...
function sanitizeFolderSegment(text) {
  return String(text || "")
    .replace(/["*:<>?/\\|,().]/g, "")  // ตัดอักขระพิเศษ + จุลภาค วงเล็บ จุด ออก
...
export async function uploadSendworkFiles(request, files = [], reviseTag = "", onProgress = null) {
  ...
  // ── ชื่อไฟล์ ──
  // Proposal: DWG-DES-{ProjectName}-Rev{N}  (ตาม spec ของ Requester)
  // อื่นๆ:    {DrawingNo}-{DrawingName}      (เหมือนเดิม)
  const isProposal = (request.requestType || "").includes("Proposal");
  let fileBaseName;
  if (isProposal) {
    const projectSafe = sanitizeFolderSegment(request.projectName || "");
    const revSafe     = sanitizeFolderSegment(reviseTag || request.currentRevise || request.reviseNumber || "R0");
    fileBaseName = projectSafe
      ? `DWG-DES-${projectSafe}-Rev${revSafe}`
      : "";
  } else {
    fileBaseName = [drawingNo, drawingName].filter(Boolean).join("-");
  }
  ...
  return Promise.all(
    fileArray.map(async (file, index) => {
      ...
      // ถ้า fileBaseName ว่าง (drawingNo และ drawingName ไม่มีข้อมูล)
      // ให้ใช้ชื่อไฟล์เดิม (ไม่ rename) เพื่อหลีกเลี่ยง Graph API 400
      const newFileName = fileBaseName
        ? `${fileBaseName}${suffix}${ext}`
        : file.name;

      const renamedFile = fileBaseName
