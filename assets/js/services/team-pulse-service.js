 ...
 *
 * เคารพ "มุมมองที่กำลังดู" ด้วย: ถ้าผู้จัดการสลับไปดูมุมมองผู้ส่งคำร้อง
 * จะต้องเห็นหน้าจอเหมือนผู้ส่งคำร้องจริง ๆ คือโพสต์ไม่ได้และไม่มีป็อปอัพเช็คอินเด้ง
 * (ถ้าไม่เช็คตรงนี้ ป็อปอัพจะเด้งทับหน้าจอทั้งที่กำลังจำลองเป็นผู้ส่งคำร้องอยู่)
 */
export function canPostTeamPulse(user) {
  if (!user?.email) return false;

  // มุมมองที่กำลังดูต้องเป็นบทบาทที่โพสต์ได้ด้วย
  const viewRole = user.role;
  if (viewRole && !["designer", "manager"].includes(viewRole)) return false;
...
/**
 * สิทธิ์ลบโพสต์ของผู้อื่น — ต้องเป็นผู้จัดการ "และ" กำลังดูในมุมมองผู้จัดการอยู่
 * ถ้าสลับไปดูมุมมองอื่น จะไม่เห็นปุ่มลบของคนอื่น เหมือนที่บทบาทนั้นเห็นจริง
 */
export function canModerateTeamPulse(user) {
  if (!user?.email) return false;
  if (user.role !== "manager") return false;
  return isTeamPulseManager(user.email);
}
