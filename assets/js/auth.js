...
function setActiveAccount(account) {
  ...
  setState({
    ...
    user: {
      ...
      // role = "มุมมองที่กำลังใช้อยู่" (ผู้จัดการสลับดูมุมมองอื่นได้)
      // actualRole = สิทธิ์จริงจาก SharePoint — ใช้ตัดสินว่าใครสลับมุมมองได้ ห้ามแก้จาก UI
      role: computeRole(email),
      actualRole: computeRole(email),
...
export function refreshUserRole() {
  ...
  const actualRole = computeRole(state.user.email);
  // ถ้าผู้จัดการกำลังสลับดูมุมมองอื่นอยู่ ห้ามรีเซ็ตมุมมองนั้นทิ้ง
  const keepViewAs = state.user.viewAs && actualRole === "manager";
  const role = keepViewAs ? state.user.viewAs : actualRole;
  setState({
    user: {
      ...state.user,
      role,
      actualRole,
      roleLabel: roleLabel(role, state.user.email),
    },
  });
}
