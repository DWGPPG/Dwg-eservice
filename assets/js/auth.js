...
/**
 * ใช้ตอนผู้ใช้กดปุ่มเอง (ไม่ใช่ตอน boot อัตโนมัติ) — ถ้า silent ล้มเหลวค่อยเปิด popup ขอ token ใหม่
...
function setActiveAccount(account) {
  ...
  setState({
    ...
    user: {
      ...
      roleLabel: roleLabel(computeRole(email)),
    },
  });
}
...
export function refreshUserRole() {
  ...
  setState({
    user: { ...state.user, role, roleLabel: roleLabel(role) },
  });
}
