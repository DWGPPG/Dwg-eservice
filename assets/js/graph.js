...
/**
 * ยิง payload เข้า Power Automate Flow (relay) — Flow จะอ่าน groupChatId แล้วโพสต์ message เข้ากลุ่มนั้น
 * ใช้ Flow เดิมของ Drawing Dept. (URL เดียวที่ยิงได้แบบไม่ต้อง OAuth) เป็นตัวส่งต่อให้ทั้ง Drawing Dept. + Nexus
 * Flow schema = { "groupChatId": string, "title": string, "message": string }
 * @param {string} webhookUrl
 * @param {object} payload  — { groupChatId, title, message }
 */
export async function sendGroupWebhook(webhookUrl, payload) {
  if (!webhookUrl) return false;
  try {
    const resp = await fetch(webhookUrl, {
      method: "POST",
