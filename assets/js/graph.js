...
/**
 * ส่งข้อความ HTML เข้ากลุ่ม Teams ผ่าน Power Automate Flow ด้วย payload กลาง { title, message }
 * ใช้กับกลุ่ม Nexus (Project Sales X DWG) สำหรับงาน Proposal ทุกขั้นตอน โดยไม่ต้องใช้ token ผู้ใช้
 * (Flow รันด้วย identity ของเจ้าของ Flow ที่เป็นสมาชิกกลุ่ม — แก้ปัญหาผู้ร้องขอไม่ได้อยู่ในกลุ่ม)
 * Flow ต้องตั้ง trigger schema = { "title": string, "message": string } แล้ว action "Post message in a chat"
 * @param {string} webhookUrl
 * @param {string} title        — ป้ายสั้นๆ (Flow จะใช้หรือไม่ใช้ก็ได้)
 * @param {string} htmlMessage  — เนื้อความ HTML ที่จะโพสต์เข้ากลุ่มจริง
 */
export async function sendGroupWebhook(webhookUrl, title, htmlMessage) {
  if (!webhookUrl) return false;
  try {
    const resp = await fetch(webhookUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title: title || "", message: htmlMessage || "" }),
    });
    if (!resp.ok) {
      const detail = await resp.text();
      console.warn("sendGroupWebhook (Power Automate) failed:", resp.status, detail);
      return false;
    }
    return true;
  } catch (error) {
    console.warn("sendGroupWebhook failed (non-critical):", error.message);
    return false;
  }
}
