...
export async function sendTeams1on1(targetEmail, htmlContent) {
  ...
  try {
    ...
    await fetch(`${GRAPH_ROOT}/chats/${chatId}/messages`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
...
async function createOrFindChat(myId, targetEmail, token) {
  ...
  if (chatResp.status === 409) {
    const err = await chatResp.json();
    let chatId = err?.error?.innerError?.id || null;
    if (!chatId) {
      const listResp = await fetch(
        `${GRAPH_ROOT}/me/chats?$filter=chatType eq 'oneOnOne'&$expand=members`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (listResp.ok) {
        const listData = await listResp.json();
        const found = listData?.value?.find((c) =>
          c.members?.some((m) => m.email?.toLowerCase() === targetEmail.toLowerCase())
        );
...
export async function sendMail(toEmail, subject, htmlBody) {
  ...
  try {
    const token = await acquireToken();
    if (!token) return false;
    await fetch(`${GRAPH_ROOT}/me/sendMail`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
